...
BEGIN
   ...
   OPEN p_CURSOR FOR
      ...
      ORDER BY SUPPLIER_TYPE, SUPPLIER_ID, POWER_FLOW_START;
...
BEGIN
   ...
   SELECT MAX(CONTRACT_ID) INTO v_CONTRACT_ID FROM INTERCHANGE_CONTRACT WHERE CONTRACT_NAME = p_SUPPLIER_ID;
   DELETE TP_CONTRACT_NUMBER
   WHERE CONTRACT_ID = v_CONTRACT_ID
      AND BEGIN_DATE = p_POWER_FLOW_START
      AND END_DATE = p_POWER_FLOW_END;
   UPDATE INTERCHANGE_CONTRACT SET (BEGIN_DATE,END_DATE) = (SELECT MIN(BEGIN_DATE), MAX(END_DATE) FROM TP_CONTRACT_NUMBER WHERE CONTRACT_ID = v_CONTRACT_ID)
   WHERE CONTRACT_ID = v_CONTRACT_ID;
   COMMIT;
...
BEGIN
   ...
   END IF;
   OPEN p_CURSOR FOR
      SELECT RECORD_CONTENT "DATA"
      FROM CDI_PJM_INSCHEDULE_CACHE
...
BEGIN
   OPEN p_CURSOR FOR
      SELECT FILTER_SUPPLIER
      FROM
         (SELECT CONSTANTS.ALL_STRING "FILTER_SUPPLIER", 0 "SORT_ORDER" FROM DUAL
         UNION ALL
         SELECT ESP_EXTERNAL_IDENTIFIER, 1 FROM ENERGY_SERVICE_PROVIDER
         ORDER BY 2, 1);
...
BEGIN
   OPEN p_CURSOR FOR
      SELECT FILTER_TARIFF
      FROM
         (SELECT CONSTANTS.ALL_STRING "FILTER_TARIFF", 0 "SORT_ORDER" FROM DUAL
         UNION 
...
--@@Remove The June 1st POLR Case Statements And Assign Energy Characteristics From The Master Table Based On The Period Usage Meter Read Begin Date.
   OPEN p_CURSOR FOR
      SELECT
         ...
         TRIM(M.TARIFF_CODE) "TARIFF_CODE",
         'BGE'               "EDC_IDENTIFIER",
         S.ESP_ID,
         U.BEGIN_DATE,
         U.END_DATE,
      ...
      WHERE U.BEGIN_DATE <= p_END_DATE
         ...
         AND M.RTO_ACCOUNT_ID LIKE '%' || TRIM(p_FILTER_ACCOUNT) || '%'
         AND (p_FILTER_TARIFF = CONSTANTS.ALL_STRING OR TO_CHAR(M.TARIFF_ID) = p_FILTER_TARIFF)
         AND ROWNUM < 25000
      GROUP BY M.RTO_ACCOUNT_ID, M.SUPPLIER, TRIM(M.TARIFF_CODE), S.ESP_ID, TRIM(M.TARIFF_CODE) || '_' || TRIM(M.POLR_TYPE) || '_' || c_DEFAULT_PLC_BAND, U.PERIOD_ID, U.PERIOD_NAME, U.TEMPLATE_ID, U.TEMPLATE_NAME, U.BEGIN_DATE, U.END_DATE
      ORDER BY 1, 2, 3, 4;
...
BEGIN
   ...
   IF p_RPT_ID IS NOT NULL THEN
      UPDATE CDI_ENERGY_INVALID_DATA SET
         ...
         BGE_BEGIN_DATE = p_BEGIN_DATE,
         BGE_END_DATE   = p_END_DATE,
...
BEGIN
   ASSERT(p_INTERVAL IS NOT NULL, 'A null value was given for INTERVAL, this field must be non-null.', MSGCODES.c_ERR_ARGUMENT);
