...
-- Cache Accounts Without An Assignment --
      INSERT INTO CDI_MISSING_PLC_NSPL_TAG_CACHE(TAG_TYPE, BILL_ACCOUNT, SERVICE_POINT, PREMISE_NUMBER, EFFECTIVE_DATE, TERMINATION_DATE, RATE_CLASS, VOLTAGE_LEVEL)
      WITH CANDIDATES AS
         (SELECT BILL_ACCOUNT, SERVICE_POINT, PREMISE_NUMBER, EFFECTIVE_DATE, TERMINATION_DATE, RATE_CLASS, VOLTAGE_LEVEL,
            ROW_NUMBER() OVER(PARTITION BY BILL_ACCOUNT, SERVICE_POINT, PREMISE_NUMBER ORDER BY BILL_ACCOUNT, SERVICE_POINT, PREMISE_NUMBER, EFFECTIVE_DATE) AS RN
         FROM BGE_CI_STAGING WHERE EFFECTIVE_DATE <> TERMINATION_DATE)
      SELECT v_TAG_TYPE, BILL_ACCOUNT, SERVICE_POINT, PREMISE_NUMBER, EFFECTIVE_DATE, TERMINATION_DATE, RATE_CLASS, VOLTAGE_LEVEL
      FROM CANDIDATES X
      WHERE RN = 1
         AND NOT EXISTS (SELECT NULL FROM CDI_PLC_ICAP_TX WHERE BILL_ACCOUNT = X.BILL_ACCOUNT AND SERVICE_POINT = X.SERVICE_POINT AND PREMISE_NUMBER = X.PREMISE_NUMBER AND SUBSTR(TAG_ID,5,1) = v_TAG_TYPE AND TO_NUMBER(SUBSTR(TAG_ID,1,4)) <= TO_NUMBER(TO_CHAR(X.EFFECTIVE_DATE,'YYYY')));
      LOGS.LOG_INFO('Number Of "' || v_PLC_NSPL.ANCILLARY_SERVICE_NAME || '" Ancillary Service Accounts Missing An Assignment: ' || TO_CHAR(SQL%ROWCOUNT));
   END LOOP;
EXCEPTION
   WHEN OTHERS THEN
