...
c_PACKAGE_NAME                 CONSTANT VARCHAR2(32) := 'CDI_BK';
...
BEGIN
   ...
   IF l_PROXY_DAY_METHOD_ID IS NULL THEN
      LOGS.LOG_WARN('The Account Proxy Day Method Is Not Specified In The System Settings (Client Data Interface -> Data Import -> Account Sync).', 'CACHE_PROXY_DAY_METHOD');
   END IF;
...
BEGIN
   ...
   WHILE v_INDEX <= p_ACCOUNT_SYNC.LAST LOOP
      LOGS.LOG_DEBUG('Processing Account: ' || p_ACCOUNT_SYNC(v_INDEX).ACCOUNT_NAME);
      ...
      IF v_NEW_ACCOUNT THEN
         LOGS.LOG_DEBUG('Detected New Account: ' || p_ACCOUNT_SYNC(v_INDEX).ACCOUNT_NAME);
   ...
   END LOOP;
   LOGS.LOG_INFO_DETAIL('Elapsed Seconds: ' || TO_CHAR(ROUND((DBMS_UTILITY.GET_TIME-v_MARK_TIME)/100)));
END SYNC_ACCOUNT;
...
BEGIN
   ...
   IF v_SERVICE_LOCATION_ID < 0 THEN
      LOGS.LOG_ERROR('Service Location "' || p_SERVICE_LOCATION_NAME || '" Is Not Defined');
   END IF;
...
BEGIN
   ...
   IF v_EDC_ID IS NULL THEN
      LOGS.LOG_ERROR('EDC Associated With External Identifier "' || p_EDC_EXTERNAL_IDENTIFIER || '" Is Not Defined');
      RETURN;
   END IF;
...
BEGIN
   ...
   WHILE v_INDEX <= p_ACCOUNT_EDC_SYNC.LAST LOOP
      ...
      IF v_EDC_ID IS NULL THEN
         LOGS.LOG_ERROR('EDC Associated With The External Identifier "' || p_ACCOUNT_EDC_SYNC(v_INDEX).EDC_IDENTIFIER || '" Is Not Defined');
      ELSE
         SELECT MAX(ACCOUNT_ID) INTO v_ACCOUNT_ID FROM ACCOUNT WHERE ACCOUNT_EXTERNAL_IDENTIFIER = v_CURRENT_KEY;
         IF v_ACCOUNT_ID IS NULL THEN
            LOGS.LOG_ERROR('Account Associated With The External Identifier "' || v_CURRENT_KEY || '" Is Not Defined');
         ELSE
            BEGIN
...
BEGIN
   ...
   WHILE v_INDEX <= p_ACCOUNT_ESP_SYNC.LAST LOOP
      ...
      IF v_ESP_ID IS NULL THEN
         LOGS.LOG_ERROR('ESP Associated With The External Identifier "' || p_ACCOUNT_ESP_SYNC(v_INDEX).ESP_IDENTIFIER || '" Is Not Defined');
      ELSE
         SELECT MAX(ACCOUNT_ID) INTO v_ACCOUNT_ID FROM ACCOUNT WHERE ACCOUNT_EXTERNAL_IDENTIFIER = v_CURRENT_KEY;
         IF v_ACCOUNT_ID IS NULL THEN
            LOGS.LOG_ERROR('Account Associated With The External Identifier "' || v_CURRENT_KEY || '" Is Not Defined');
         ELSE
            ...
            IF v_POOL_ID IS NULL THEN
               LOGS.LOG_ERROR('Pool Associated With The External Identifier "' || p_ACCOUNT_ESP_SYNC(v_INDEX).POOL_IDENTIFIER || '" Is Not Defined');
            ELSE
               BEGIN
                  SELECT ROWID, BEGIN_DATE, END_DATE INTO v_ROWID, v_OLD_BEGIN_DATE, v_OLD_END_DATE
                  FROM ACCOUNT_ESP
               ...
               ELSE
                  ...
                  EXCEPTION
                     WHEN DUP_VAL_ON_INDEX THEN
                        SELECT MAX(ACCOUNT_NAME) INTO v_ACCOUNT_NAME FROM ACCOUNT WHERE ACCOUNT_ID = v_ACCOUNT_ID;
                        LOGS.LOG_ERROR('Duplicate Supplier Assignments Detected For Account: ' || v_ACCOUNT_NAME || ' On Begin Date: ' || TO_CHAR(v_NEW_BEGIN_DATE, c_DATE_FORMAT));
                     WHEN OTHERS THEN
                        ERRS.LOG_AND_RAISE;
                  END;      
               END IF;
...
BEGIN
   ...
   WHILE v_INDEX <= p_ACCOUNT_LOSS_FACTOR_SYNC.LAST LOOP
      ...
      IF v_LOSS_FACTOR_ID IS NULL THEN
         LOGS.LOG_ERROR('Loss Factor Associated With The External Identifier "' || p_ACCOUNT_LOSS_FACTOR_SYNC(v_INDEX).LOSS_FACTOR_NAME || '" Is Not Defined');
      ELSE
         SELECT MAX(ACCOUNT_ID) INTO v_ACCOUNT_ID FROM ACCOUNT WHERE ACCOUNT_EXTERNAL_IDENTIFIER = v_CURRENT_KEY;
         IF v_ACCOUNT_ID IS NULL THEN
            LOGS.LOG_ERROR('Account Associated With The External Identifier "' || v_CURRENT_KEY || '" Is Not Defined');
   ...
   WHILE v_INDEX <= p_ACCOUNT_CALENDAR_SYNC.LAST LOOP
      ...
      ELSE
         ...
         IF v_CALENDAR_ID IS NULL THEN
            LOGS.LOG_ERROR('Calendar Associated With The External Identifier "' || p_ACCOUNT_CALENDAR_SYNC(v_INDEX).CALENDAR_NAME || '" Is Not Defined');
         ELSE
            BEGIN
               SELECT ROWID, BEGIN_DATE, END_DATE INTO v_ROWID, v_OLD_BEGIN_DATE, v_OLD_END_DATE
   ...
   WHILE v_INDEX <= p_ACCOUNT_PRODUCT_SYNC.LAST LOOP
      ...
      ELSE
         ...
         IF v_PRODUCT_ID IS NULL THEN
            LOGS.LOG_ERROR('Product Associated With The External Identifier "' || p_ACCOUNT_PRODUCT_SYNC(v_INDEX).PRODUCT_EXTERNAL_IDENTIFIER || '" Is Not Defined');
         ELSE
            BEGIN
               SELECT ROWID, BEGIN_DATE, END_DATE INTO v_ROWID, v_OLD_BEGIN_DATE, v_OLD_END_DATE
...
BEGIN
   ...
   IF v_STATION_ID < 0 THEN
      LOGS.LOG_ERROR('Weather Station Associated With The Identifier "' || p_STATION_NAME || '" Is Not Defined');
   ELSE
      ID.ID_FOR_WEATHER_PARAMETER(p_PARAMETER_NAME, v_PARAMETER_ID);
      IF v_PARAMETER_ID < 0 THEN
         LOGS.LOG_ERROR('Weather Parameter Associated With The Identifier "' || p_PARAMETER_NAME || '" Is Not Defined');
      ELSE
...
BEGIN
   ...
   END LOOP;
   LOGS.LOG_INFO_DETAIL('Insert Count: ' || TO_CHAR(v_INSERT) || ', Update Count: ' || TO_CHAR(v_UPDATE) || ', Elapsed Seconds: ' || TO_CHAR(ROUND((DBMS_UTILITY.GET_TIME-v_MARK_TIME)/100)));
END SYNC_POOL;
...
BEGIN
   ...
   IF v_CALENDAR_ID < 0 THEN
      LOGS.LOG_ERROR('Calendar Associated With The Identifier "' || p_CALENDAR_NAME || '" Is Not Defined');
   ELSE
   ...
   WHILE v_INDEX <= p_ACCOUNT_SERV_LOC_SYNC.LAST LOOP
      ...
      ELSE
         ...
         IF v_SERVICE_LOCATION_ID IS NULL THEN
            LOGS.LOG_ERROR('Service Location Associated With The External Identifier "' || p_ACCOUNT_SERV_LOC_SYNC(v_INDEX).SERV_LOC_EXTERNAL_IDENTIFIER || '" Is Not Defined');
         ELSE
            BEGIN
               SELECT ROWID, BEGIN_DATE, END_DATE INTO v_ROWID, v_OLD_BEGIN_DATE, v_OLD_END_DATE
