...
PROCEDURE DEFINE_CONTRACT
   ...
   p_CONTRACT_ID        IN NUMBER, --@@PJM_BASE_ID
   p_BEGIN_DATE         IN DATE,   --@@POWER_FLOW_START
   p_END_DATE           IN DATE,   --@@POWER_FLOW_END
   p_PSE_ID             IN NUMBER,
...
v_END_DATE DATE;
BEGIN
   SELECT NVL(MAX(CONTRACT_ID), CONSTANTS.NOT_ASSIGNED), NVL(MAX(BEGIN_DATE), CONSTANTS.HIGH_DATE), NVL(MAX(END_DATE), CONSTANTS.LOW_DATE) INTO v_CONTRACT_ID, v_BEGIN_DATE, v_END_DATE FROM INTERCHANGE_CONTRACT WHERE CONTRACT_NAME = v_CONTRACT_NAME;
   LOGS.LOG_DEBUG('CONTRACT_TYPE: ' || p_CONTRACT_TYPE || ', SUPPLIER_ID: ' || p_SUPPLIER_ID || ', SHORT_NAME: ' || p_SHORT_NAME || ', CONTRACT_ID: ' || TO_CHAR(p_CONTRACT_ID) || ', BEGIN_DATE: ' || TO_CHAR(p_BEGIN_DATE, c_DATE_FORMAT) || ', END_DATE: ' || TO_CHAR(p_END_DATE, c_DATE_FORMAT) || ', PSE_ID: ' || TO_CHAR(p_PSE_ID) || ', INC_LIMIT_QUANTITY: ' || TO_CHAR(p_INC_LIMIT_QUANTITY) || ', DEC_LIMIT_QUANTITY: ' || TO_CHAR(p_INC_LIMIT_QUANTITY) || ', CONTRACT_ID: ' || TO_CHAR(v_CONTRACT_ID) || ', CONTRACT_NAME: ' || v_CONTRACT_NAME);
   IO.PUT_INTERCHANGE_CONTRACT(
      ...
      p_BEGIN_DATE           => LEAST(p_BEGIN_DATE, v_BEGIN_DATE), --@@Contract Begin Date Must Maintain Any Existing Begin Date--
      p_END_DATE             => GREATEST(p_END_DATE, v_END_DATE),  --@@Contract End Date Must Maintain Any Existing End Date--
      p_IS_EVERGREEN         => NULL,
   ...
   IF p_CONTRACT_TYPE = 'INC' THEN
      ...
      END IF;
   END IF;
   SELECT MAX(BEGIN_DATE) INTO v_BEGIN_DATE FROM TP_CONTRACT_NUMBER WHERE CONTRACT_ID = v_CONTRACT_ID AND TP_ID = l_TP_ID;
...
CURSOR c_SELECT IS
   ...
   ORDER BY POWER_FLOW_START, SUPPLIER_ID;
...
BEGIN
   SECURITY_CONTROLS.SET_CURRENT_USER(SECURITY_CONTROLS.c_SUSER_SYSTEM);
   CDI_PERIOD_USAGE.LOAD_PERIOD_USAGE(v_STATUS, v_MESSAGE);
...
PROCEDURE INITIAL_BID_BLOCK_SIZE AS
v_BEGIN_DATE DATE := TRUNC(CURRENT_DATE);
v_END_DATE   DATE := v_BEGIN_DATE;
v_STATUS  NUMBER;
v_MESSAGE VARCHAR2(1000);
BEGIN
   SECURITY_CONTROLS.SET_CURRENT_USER(SECURITY_CONTROLS.c_SUSER_SYSTEM);
   CDI_ANCILLARY_SERVICE.SET_INITIAL_BID_BLOCK_SIZE(v_BEGIN_DATE, v_END_DATE, v_STATUS, v_MESSAGE);
END INITIAL_BID_BLOCK_SIZE;
