...
TYPE r_ESCHEDULE IS RECORD (CONTRACT_NUMBER NUMBER(16), SCHEDULE_DATE DATE, LOCAL_DAY DATE, ALT_LOCAL_HOUR NUMBER(2), IS_DST_SPRING_AHEAD_DAY NUMBER(1), IS_DST_FALL_BACK_DAY NUMBER(1), AMOUNT NUMBER(14,4), PRIOR_CONTRACT_NUMBER NUMBER(16), PRIOR_LOCAL_DAY DATE);
...
c_SCHEDULE_ROUND_PLACE         CONSTANT NUMBER(2)    := 3;
...
-- Collect The Schedule Content To Be Processed --
   SELECT CONTRACT_SCHEDULE_TYPE(A.CONTRACT_ID, CASE WHEN l_ROLL_UP_BY_CONTRACT = 1 THEN CONSTANTS.NOT_ASSIGNED ELSE A.TRANSACTION_ID END, C.SCHEDULE_DATE,
      ROUND(CASE WHEN p_SCHEDULE_TYPE = CONSTANTS.SCHEDULE_TYPE_FINAL THEN CASE WHEN l_ENABLE_RECON_DELTA_MODE = 1 THEN (NVL(D.AMOUNT,0)-NVL(C.AMOUNT,0)) * 1000 ELSE NVL(D.AMOUNT,0) END ELSE NVL(C.AMOUNT,0) END, c_SCHEDULE_ROUND_PLACE))
...
-- Collect And Order The Schedule Content Into A Cursor Based On The Roll-Up Option --
   IF l_ROLL_UP_BY_CONTRACT = 0 THEN
      OPEN p_CURSOR FOR
         SELECT CONTRACT_NUMBER, SCHEDULE_DATE, LOCAL_DAY, ALT_LOCAL_HOUR, IS_DST_SPRING_AHEAD_DAY, IS_DST_FALL_BACK_DAY, CASE WHEN v_FLOOR_TO_ZERO = 1 THEN GREATEST(AMOUNT,0) ELSE AMOUNT END "AMOUNT",
         ...
         FROM
            (SELECT B.TRANSACTION_IDENTIFIER "CONTRACT_NUMBER", A.SCHEDULE_DATE, C.LOCAL_DAY, C.ALT_LOCAL_HOUR, C.IS_DST_SPRING_AHEAD_DAY, C.IS_DST_FALL_BACK_DAY, SUM(A.SCHEDULE_VAL) "AMOUNT"
            ...
            GROUP BY B.TRANSACTION_IDENTIFIER, A.SCHEDULE_DATE, C.LOCAL_DAY, C.ALT_LOCAL_HOUR, C.IS_DST_SPRING_AHEAD_DAY, C.IS_DST_FALL_BACK_DAY)
         ORDER BY CONTRACT_NUMBER, LOCAL_DAY, ALT_LOCAL_HOUR;
   ELSE
      OPEN p_CURSOR FOR
         SELECT CONTRACT_NUMBER, SCHEDULE_DATE, LOCAL_DAY, ALT_LOCAL_HOUR, IS_DST_SPRING_AHEAD_DAY, IS_DST_FALL_BACK_DAY, CASE WHEN v_FLOOR_TO_ZERO = 1 THEN GREATEST(AMOUNT,0) ELSE AMOUNT END "AMOUNT",
            LAG(CONTRACT_NUMBER, 1, CONTRACT_NUMBER) OVER (ORDER BY CONTRACT_NUMBER) "PRIOR_CONTRACT_NUMBER",
         ...
         FROM
            (SELECT B.CONTRACT_NUMBER, A.SCHEDULE_DATE, C.LOCAL_DAY, C.ALT_LOCAL_HOUR, C.IS_DST_SPRING_AHEAD_DAY, C.IS_DST_FALL_BACK_DAY, SUM(A.SCHEDULE_VAL) "AMOUNT"
            ...
            GROUP BY B.CONTRACT_NUMBER, A.SCHEDULE_DATE, C.LOCAL_DAY, C.ALT_LOCAL_HOUR, C.IS_DST_SPRING_AHEAD_DAY, C.IS_DST_FALL_BACK_DAY)
         ORDER BY CONTRACT_NUMBER, LOCAL_DAY, ALT_LOCAL_HOUR;
   END IF;
...
v_ESCHEDULE_FORMAT CDI_ESCHEDULE_FORMAT_LIST := CDI_ESCHEDULE_FORMAT_LIST();
   FUNCTION POST_HOUR_SPEC(p_LOCAL_HOUR IN NUMBER, p_IS_DST_SPRING_AHEAD_DAY IN NUMBER, p_IS_DST_FALL_BACK_DAY IN NUMBER, p_AMOUNT IN NUMBER) RETURN VARCHAR2 AS
   v_HOUR_SPEC VARCHAR2(64);
   BEGIN
      RETURN SUBSTR('0' || TO_CHAR(p_LOCAL_HOUR-1),-2) || '-' || SUBSTR('0' || TO_CHAR(p_LOCAL_HOUR+CASE WHEN p_IS_DST_SPRING_AHEAD_DAY = 1 AND p_LOCAL_HOUR = 2 THEN 1 ELSE 0 END),-2) || ' ' || TO_CHAR(p_AMOUNT) || c_CRLF;
   END POST_HOUR_SPEC;
BEGIN
   LOGS.LOG_INFO('Entry');
   p_COUNT := 0;
   ...
   LOOP
      ...
      IF c_FILE_FORMAT_OPTION = 'A' THEN
         IF v_ESCHEDULE.CONTRACT_NUMBER <> v_ESCHEDULE.PRIOR_CONTRACT_NUMBER OR v_ESCHEDULE.LOCAL_DAY <> v_ESCHEDULE.PRIOR_LOCAL_DAY THEN
            v_BUFFER := v_HEADER || c_CRLF || v_ESCHEDULE.CONTRACT_NUMBER || c_CRLF || TO_CHAR(v_ESCHEDULE.LOCAL_DAY, c_DATE_FORMAT) || c_CRLF || TO_CHAR(v_ESCHEDULE.LOCAL_DAY, c_DATE_FORMAT) || c_CRLF || POST_HOUR_SPEC(v_ESCHEDULE.ALT_LOCAL_HOUR, v_ESCHEDULE.IS_DST_SPRING_AHEAD_DAY, v_ESCHEDULE.IS_DST_FALL_BACK_DAY, v_ESCHEDULE.AMOUNT);
            DBMS_LOB.WRITEAPPEND(p_CONTENT, LENGTH(v_BUFFER), v_BUFFER);
            p_COUNT := p_COUNT + 5;
         ELSE
            v_BUFFER := POST_HOUR_SPEC(v_ESCHEDULE.ALT_LOCAL_HOUR, v_ESCHEDULE.IS_DST_SPRING_AHEAD_DAY, v_ESCHEDULE.IS_DST_FALL_BACK_DAY, v_ESCHEDULE.AMOUNT);
            DBMS_LOB.WRITEAPPEND(p_CONTENT, LENGTH(v_BUFFER), v_BUFFER);
            p_COUNT := p_COUNT + 1;
         END IF;
...
PROCEDURE CACHE_SUBMITTED_CONTENT(p_CONTENT IN CLOB) AS
v_CONTENT PARSE_UTIL.BIG_STRING_TABLE_MP;
v_COUNT PLS_INTEGER := 0;
BEGIN
   EXECUTE IMMEDIATE 'TRUNCATE TABLE CDI_PJM_INSCHEDULE_CACHE';
   PARSE_UTIL.PARSE_CLOB_INTO_LINES(p_CONTENT, v_CONTENT);
   FOR v_INDEX IN v_CONTENT.FIRST..v_CONTENT.LAST LOOP
      v_COUNT := v_COUNT + 1;
      INSERT INTO CDI_PJM_INSCHEDULE_CACHE(RECORD_NUMBER, RECORD_CONTENT)
      VALUES(v_COUNT, v_CONTENT(v_INDEX));
   END LOOP;
   COMMIT;
END CACHE_SUBMITTED_CONTENT;
...
-- Save The Content Sent To PJM In A Review Table --
   CACHE_SUBMITTED_CONTENT(v_CONTENT);   
...
-- Save The Content In A Review Table --
   CACHE_SUBMITTED_CONTENT(p_FILE);   
...
-- Stage The System Load --
   ...
   FOR v_SELECT IN c_SYSTEM_LOAD(v_CUT_BEGIN_DATE, v_CUT_END_DATE) LOOP
      ...
      v_BUFFER := TO_CHAR(v_SELECT.PRELIM) || ',' || TO_CHAR(ROUND(v_SELECT.PRELIM * v_TX_LOSS_FACTOR, c_SCHEDULE_ROUND_PLACE)) || ',' || TO_CHAR(v_SELECT.FINAL) || ',' || TO_CHAR(ROUND(v_SELECT.FINAL * v_TX_LOSS_FACTOR, c_SCHEDULE_ROUND_PLACE)) || ',' || TO_CHAR(v_SELECT.PRELIM - v_SELECT.FINAL) || ',' || TO_CHAR(ROUND((v_SELECT.PRELIM - v_SELECT.FINAL) * v_TX_LOSS_FACTOR, c_SCHEDULE_ROUND_PLACE)) || ',';
...
-- Stage The Supplier Load --
      FOR v_SELECT IN c_SUPPLIER_LOAD(v_SUPPLIER.ESP_ID, v_CUT_BEGIN_DATE, v_CUT_END_DATE) LOOP
         v_TX_LOSS_FACTOR := GET_LOSS_FACTOR(v_SELECT.LOCAL_DATE);
         v_BUFFER := TO_CHAR(v_SELECT.PRELIM) || ',' || TO_CHAR(ROUND(v_SELECT.PRELIM * v_TX_LOSS_FACTOR, c_SCHEDULE_ROUND_PLACE)) || ',' || TO_CHAR(v_SELECT.FINAL) || ',' || TO_CHAR(ROUND(v_SELECT.FINAL * v_TX_LOSS_FACTOR, c_SCHEDULE_ROUND_PLACE)) || ',' || TO_CHAR(ROUND((v_SELECT.PRELIM - v_SELECT.FINAL) * 1000)) || ',' || TO_CHAR(ROUND((v_SELECT.PRELIM - v_SELECT.FINAL) * v_TX_LOSS_FACTOR * 1000)) || ',';
