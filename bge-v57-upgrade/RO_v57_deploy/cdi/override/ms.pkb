...
gc_CLEANUP_SERVICE_STATE   CONSTANT NUMBER(1)    := 1;
...
-- Get Account/Meter Info
		...
		--Check Template, only allow Not Assigned, Anytime, and Anytime TOU
...
FUNCTION GET_SERVICE_STATE
    ...
    p_CLEANUP_SERVICE_STATE IN NUMBER := 0
	) RETURN SERVICE_STATE%ROWTYPE IS
  v_SERVICE_STATE_LIST NUMBER_COLLECTION := number_collection();
  ...
  v_IDX NUMBER;
BEGIN
  IF p_CLEANUP_SERVICE_STATE = 1 THEN
       ...
       -- get the ESP IDs and associated POOL IDs for this account and service date
        ...
        v_IDX := v_ESP_IDS.FIRST;
        WHILE v_ESP_IDS.EXISTS(v_IDX) LOOP
		   v_SERVICE_STATE_LIST.EXTEND;
           v_SERVICE_STATE_LIST(v_IDX) := CS.GET_SERVICE_STATE(GA.DEFAULT_MODEL, p_SCENARIO_ID, p_ACCOUNT_ID, p_SERVICE_LOCATION_ID, p_METER_ID, p_AGGREGATE_ID, p_SERVICE_CODE, p_SERVICE_DATE, p_AS_OF_DATE,
											p_EDC_ID, v_ESP_IDS(v_IDX), p_PSE_ID, v_POOL_IDS(v_IDX), g_APPLY_UFE_CODE, p_REQUESTOR).SERVICE_ID;

           v_IDX := v_ESP_IDS.NEXT(v_IDX);
        END LOOP;
  ELSE
    v_SERVICE_STATE_LIST.EXTEND;
    v_SERVICE_STATE_LIST(1) := CS.GET_SERVICE_STATE(GA.DEFAULT_MODEL, p_SCENARIO_ID, p_ACCOUNT_ID, p_SERVICE_LOCATION_ID, p_METER_ID, p_AGGREGATE_ID, p_SERVICE_CODE, p_SERVICE_DATE, p_AS_OF_DATE,
											p_EDC_ID, p_ESP_ID, p_PSE_ID, p_POOL_ID, g_APPLY_UFE_CODE, p_REQUESTOR).SERVICE_ID;
  END IF;
  ...
  IF p_CLEANUP_SERVICE_STATE = 1 OR NOT GA.CSB_IS_SUBDAILY THEN
		DELETE FROM SERVICE_STATE SS WHERE
			SS.SERVICE_ID IN (SELECT SRV.SERVICE_ID
								...
								WHERE CURR_SRV.SERVICE_ID = v_SERVICE_STATE.SERVICE_ID
									...
									AND SRV.SERVICE_ID NOT IN (SELECT COLUMN_VALUE FROM TABLE(CAST(v_SERVICE_STATE_LIST AS NUMBER_COLLECTION)) SSL ))
	...
	IF p_CLEANUP_SERVICE_STATE = 1 AND GA.CSB_IS_SUBDAILY AND p_DELETE_FIRST = 1 THEN
   DELETE FROM SERVICE_STATE SS WHERE
			SS.SERVICE_ID IN (SELECT COLUMN_VALUE FROM TABLE(CAST(v_SERVICE_STATE_LIST AS NUMBER_COLLECTION)) SSL )
				AND SS.SERVICE_CODE = v_SERVICE_STATE.SERVICE_CODE
				AND SS.SERVICE_DATE = v_SERVICE_STATE.SERVICE_DATE;
	END IF;
...
-- Determine the correct Time Zone
	...
	v_SERVICE_LOAD.LOAD_DATE := FROM_TZ(CAST(v_LOCAL_DATE AS TIMESTAMP), RO_TZ_OFFSET(v_TIME_ZONE)) AT TIME ZONE RO_TZ_OFFSET(CUT_TIME_ZONE);
...
BEGIN
	...
	v_CUT_DATE := FROM_TZ(CAST(p_USAGE_DATE AS TIMESTAMP), RO_TZ_OFFSET(v_TIME_ZONE)) AT TIME ZONE RO_TZ_OFFSET(CUT_TIME_ZONE);
...
BEGIN
	...
	v_SERVICE_STATE1 := GET_SERVICE_STATE(p_SCENARIO_ID,
										  ...
										  p_AS_OF_DATE,
										  g_UNKNOWN,
                                          p_CLEANUP_SERVICE_STATE => gc_CLEANUP_SERVICE_STATE);
	...
	IF p_SUCCESS THEN
		...
		IF v_LOAD_USAGE_DATA THEN
			v_SERVICE_STATE := GET_SERVICE_STATE(v_SCENARIO_ID, p_SERVICE_CODE,
								...
								TRUNC(p_SERVICE_DATE), p_AS_OF_DATE, g_UNKNOWN, p_DELETE_FIRST, gc_CLEANUP_SERVICE_STATE);
...
BEGIN
	...
	v_AREA_LOAD.LOAD_DATE := FROM_TZ(CAST(v_LOCAL_DATE AS TIMESTAMP), RO_TZ_OFFSET(v_TIME_ZONE)) AT TIME ZONE RO_TZ_OFFSET(CUT_TIME_ZONE);
...
PROCEDURE IMPORT_PROVIDER_CONSUMPTION
   ...
   p_BILLED_USAGE                IN VARCHAR2,
   p_METERS_READ                 IN VARCHAR2,
   p_METERED_USAGE               IN VARCHAR2,
   p_PERIOD_ID                   IN NUMBER,
   p_TEMPLATE_ID                 IN NUMBER,
   p_RECEIVED_DATE               IN DATE,
   p_TRACE_ON                    IN NUMBER,
...
BEGIN
   ...
   IF LOGS.IS_DEBUG_ENABLED THEN
      ...
      LOGS.LOG_DEBUG('ESP_EXTERNAL_IDENTIFIER: '     || p_ESP_EXTERNAL_IDENTIFIER);
      LOGS.LOG_DEBUG('PSE_EXTERNAL_IDENTIFIER: '     || p_PSE_EXTERNAL_IDENTIFIER);
      LOGS.LOG_DEBUG('BEGIN_DATE: '   || TO_CHAR(p_BEGIN_DATE,'MM/DD/YYYY'));
      LOGS.LOG_DEBUG('END_DATE: '     || TO_CHAR(p_END_DATE,'MM/DD/YYYY'));
      LOGS.LOG_DEBUG('DATE_FORMAT: '  || p_DATE_FORMAT);
      LOGS.LOG_DEBUG('BILLED_USAGE: ' || p_BILLED_USAGE);
   END IF;
   ...
   ID.ID_FOR_ACCOUNT_EXTERNAL_IDENT(p_ACCOUNT_EXTERNAL_IDENTIFIER, FALSE, v_ACCOUNT_ID);
   IF v_ACCOUNT_ID <= 0 THEN
      ERRS.LOG_AND_RAISE('Account Not Defined For ' || p_ACCOUNT_EXTERNAL_IDENTIFIER);
   END IF;

   v_EDC_ID := ID.ID_FOR_EDC_EXTERNAL_IDENTIFIER(p_EDC_EXTERNAL_IDENTIFIER);
   IF v_EDC_ID <= 0 THEN
      ERRS.LOG_AND_RAISE('EDC Not Defined For ' || p_EDC_EXTERNAL_IDENTIFIER);
   END IF;

   v_ESP_ID := ID.ID_FOR_ESP_EXTERNAL_IDENTIFIER(p_ESP_EXTERNAL_IDENTIFIER);
   IF v_ESP_ID <= 0 THEN
      ERRS.LOG_AND_RAISE('ESP Not Defined For ' || p_ESP_EXTERNAL_IDENTIFIER);
   END IF;

   v_PSE_ID := ID.ID_FOR_PSE_EXTERNAL_IDENTIFIER(p_PSE_EXTERNAL_IDENTIFIER);
   IF v_PSE_ID <= 0 THEN
      ERRS.LOG_AND_RAISE('PSE Not Defined For ' || p_PSE_EXTERNAL_IDENTIFIER);
   END IF;

   v_POOL_ID := ID.ID_FOR_POOL_EXTERNAL_IDENT(p_POOL_EXTERNAL_IDENTIFIER);
   IF v_POOL_ID <= 0 THEN
      ERRS.LOG_AND_RAISE('Pool Not Defined For ' || p_POOL_EXTERNAL_IDENTIFIER);
   END IF;

   v_SERVICE_LOCATION_ID := CS.GET_SERVICE_LOCATION_ID(v_ACCOUNT_ID, v_BEGIN_DATE);
   IF v_SERVICE_LOCATION_ID <= 0 THEN
      ERRS.LOG_AND_RAISE('Service Location Not Defined For Account ' || p_ACCOUNT_EXTERNAL_IDENTIFIER);
   END IF;

   SELECT MAX(IS_AGGREGATE_ACCOUNT) INTO v_IS_AGGREGATE_ACCOUNT FROM ACCOUNT WHERE ACCOUNT_ID = v_ACCOUNT_ID;
   IF v_IS_AGGREGATE_ACCOUNT = 0 THEN
      v_AGGREGATE_ID := 0;
   ELSE
      v_AGGREGATE_ID := GET_AGGREGATE_ID(v_ACCOUNT_ID, v_ESP_ID, v_BEGIN_DATE, v_END_DATE, v_POOL_ID);
      IF v_AGGREGATE_ID = CONSTANTS.NOT_ASSIGNED THEN
         IF GA.ENABLE_AGG_POST_ESP_ASSIGNMENT THEN
            ...
            v_BEGIN_DATE, v_END_DATE, v_AGGREGATE_ID);
         ELSE
            ERRS.LOG_AND_RAISE('Aggregate ESP Not Defined For ' || p_ESP_EXTERNAL_IDENTIFIER);
         END IF;
       END IF;
   END IF;
   ...
   PUT_SERVICE_CONSUMPTION(GA.DEFAULT_MODEL, v_SCENARIO_ID, v_ACCOUNT_SERVICE_ID, v_PROVIDER_SERVICE_ID, v_BEGIN_DATE, v_END_DATE, v_METER_TYPE, v_BILLED_USAGE, NULL, v_METERED_USAGE, NULL, v_METERS_READ, GA.BILL_CONSUMPTION, GA.ACTUAL_CONSUMPTION, CONSTANTS.NOT_ASSIGNED, p_RECEIVED_DATE, p_TEMPLATE_ID, p_PERIOD_ID, v_POOL_ID, NULL, v_CONSUMPTION_ID);
...
v_SERVICE_STATE SERVICE_STATE%ROWTYPE;
BEGIN
   v_SERVICE_STATE := CS.GET_PROV_POOL_SERVICE_STATE(Ga.DEFAULT_MODEL, Ga.BASE_SCENARIO_ID, p_ACCOUNT_SERVICE_ID, p_PROVIDER_SERVICE_ID, p_POOL_ID, p_SERVICE_DATE, p_AS_OF_DATE, p_UFE_FLAG);
   CS.PUT_SERVICE_STATE(v_SERVICE_STATE);
   RETURN v_SERVICE_STATE;
END GET_PROV_POOL_SERVICE_STATE;

PROCEDURE IMP_PROVIDER_POOL_INTL_USAGE
   (
   p_MODEL_ID                    IN NUMBER,
   p_ACCOUNT_EXTERNAL_IDENTIFIER IN VARCHAR2,
   p_EDC_EXTERNAL_IDENTIFIER     IN VARCHAR2,
   ...
   p_PSE_EXTERNAL_IDENTIFIER     IN VARCHAR2,
   p_POOL_EXTERNAL_IDENTIFIER    IN VARCHAR2,
   p_USAGE_DATE                  IN VARCHAR2,
   p_TIME_ZONE                   IN VARCHAR2,
   p_DATE_FORMAT                 IN VARCHAR2 DEFAULT 'DD-MON-YYYY',
   p_INTERVAL                    IN VARCHAR2 DEFAULT 3600,
   p_USAGE_VALUES                IN VARCHAR2,
   p_TRANSMISSION_LOSS_FACTOR    IN NUMBER,
   p_DISTRIBUTION_LOSS_FACTOR    IN NUMBER,
   p_DST_PROCESS_FLAG            IN NUMBER DEFAULT 1,
...
v_INTERVAL NUMBER;
v_STRING_TABLE GA.STRING_TABLE;
v_DST_STRING_TABLE GA.STRING_TABLE;
v_AS_OF_DATE DATE := SYSDATE;
v_IS_DST BOOLEAN := INSTR(p_TIME_ZONE,'D') > 0;
v_ACCOUNT_SERVICE_ID NUMBER;
v_PROVIDER_SERVICE_ID NUMBER;
...
BEGIN
   ...
   p_STATUS := GA.SUCCESS;
   p_MESSAGE := 'OK';
   v_LOCAL_DATE := ADD_SECONDS_TO_DATE(TO_DATE(p_USAGE_DATE, p_DATE_FORMAT), p_INTERVAL) ;
   v_CUT_DATE := TO_CUT(v_LOCAL_DATE, p_TIME_ZONE);
   ...
   IF LOGS.IS_DEBUG_DETAIL_ENABLED THEN
      LOGS.LOG_DEBUG_DETAIL('IMP_PROVIDER_POOL_INTL_USAGE');
      LOGS.LOG_DEBUG_DETAIL('ACCOUNT_EXTERNAL_IDENTIFIER: ' || p_ACCOUNT_EXTERNAL_IDENTIFIER);
      LOGS.LOG_DEBUG_DETAIL('EDC_EXTERNAL_IDENTIFIER: '     || p_EDC_EXTERNAL_IDENTIFIER);
      LOGS.LOG_DEBUG_DETAIL('ESP_EXTERNAL_IDENTIFIER: '     || p_ESP_EXTERNAL_IDENTIFIER);
      LOGS.LOG_DEBUG_DETAIL('PSE_EXTERNAL_IDENTIFIER: '     || p_PSE_EXTERNAL_IDENTIFIER);
      LOGS.LOG_DEBUG_DETAIL('POOL_EXTERNAL_IDENTIFIER: '    || p_POOL_EXTERNAL_IDENTIFIER);
      LOGS.LOG_DEBUG_DETAIL('USAGE_DATE: '  || TO_CHAR(p_USAGE_DATE,'MM/DD/YYYY'));
      LOGS.LOG_DEBUG_DETAIL('TIME_ZONE: '   || p_TIME_ZONE);
      LOGS.LOG_DEBUG_DETAIL('DATE_FORMAT: ' || p_DATE_FORMAT);
      LOGS.LOG_DEBUG_DETAIL('INTERVAL: '    || p_INTERVAL);
      LOGS.LOG_DEBUG_DETAIL('<index>,<service id>,<date>,<load>,<tx loss>,<dx loss>');
   END IF;
   ...
   CS.GET_ACCOUNT_SERVICE_ID(v_ACCOUNT_ID, v_SERVICE_LOCATION_ID, CONSTANTS.NOT_ASSIGNED, CONSTANTS.NOT_ASSIGNED, v_ACCOUNT_SERVICE_ID);
   CS.GET_PROVIDER_SERVICE_ID(v_EDC_ID, v_ESP_ID, v_PSE_ID, v_PROVIDER_SERVICE_ID);
   v_UFE_FLAG := CX.GET_IS_UFE_PARTICIPANT(v_ACCOUNT_ID);
   v_SERVICE_STATE := GET_PROV_POOL_SERVICE_STATE(v_ACCOUNT_SERVICE_ID, v_PROVIDER_SERVICE_ID, v_POOL_ID, TRUNC(v_LOCAL_DATE), v_AS_OF_DATE, v_UFE_FLAG);
   IF v_SERVICE_STATE.SERVICE_ID <= 0 THEN
      ERRS.LOG_AND_RAISE('Account Service Not Defined For ' || p_ACCOUNT_EXTERNAL_IDENTIFIER);
   END IF;

   UT.TOKENS_FROM_STRING(p_USAGE_VALUES,',', v_STRING_TABLE);
   v_IS_ADJUSTED_DAY := FALSE;

   IF v_IS_DST AND v_STRING_TABLE.COUNT = 24 AND p_dst_process_flag = 1 THEN
      IF TRUNC(v_CUT_DATE) = TRUNC(DST_SPRING_AHEAD_DATE(v_CUT_DATE)) THEN
         v_IS_ADJUSTED_DAY := TRUE;
         IF GA.DST_SPRING_AHEAD_OPTION = 'A' THEN
            v_STRING_TABLE.DELETE(2);
   ...
   FOR v_INDEX IN v_STRING_TABLE.FIRST..v_STRING_TABLE.LAST LOOP
      IF v_STRING_TABLE.EXISTS(v_INDEX) THEN
         ...
         v_UE_LOSS(v_INDEX) := CONSTANTS.NOT_ASSIGNED;
         v_CUT_DATE := GET_NEXT_CUT_DATE(v_CUT_DATE, v_INTERVAL, v_IS_ADJUSTED_DAY);
         LOGS.LOG_DEBUG_DETAIL(TO_CHAR(v_INDEX) || ',' || TO_CHAR(v_SERVICE_STATE.SERVICE_ID) || ',' || UT.TRACE_DATE(v_USAGE_DATE(v_INDEX)) || ',' || TO_CHAR(v_USAGE_VAL(v_INDEX)) || ',' || TO_CHAR(v_TX_LOSS(v_INDEX)) || ',' || TO_CHAR(v_DX_LOSS(v_INDEX)));
      END IF;
   END LOOP;

   STORE_SERVICE_LOAD(v_SERVICE_STATE.SERVICE_ID, GA.ACTUAL_SERVICE, v_USAGE_DATE, v_USAGE_VAL, v_TX_LOSS, v_DX_LOSS, v_UE_LOSS);
   p_SERVICE_ID := v_SERVICE_STATE.SERVICE_ID;
   
EXCEPTION
   WHEN OTHERS THEN
      p_STATUS := SQLCODE;
      p_MESSAGE := SQLERRM;
END IMP_PROVIDER_POOL_INTL_USAGE;
