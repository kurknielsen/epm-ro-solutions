...
BEGIN
	...
	v_TZ_BEGIN_DATE := FROM_TZ(CAST(TRUNC(p_BEGIN_DATE) AS TIMESTAMP), RO_TZ_OFFSET(GA.CUT_TIME_ZONE)) AT TIME ZONE RO_TZ_OFFSET(p_TIME_ZONE);
...
--@@Begin Implementation Overrid--
--@@Retrieve Schedule Type 2 And 3 Instead Of 1 And 3--
		GET_SCHEDULE(p_TRANSACTION_IDS, 2, CONSTANTS.INTERNAL_STATE, p_BEGIN_DATE, p_END_DATE, p_AS_OF_DATE, 'EDT', v_FORECAST_SCHEDULE, CASE WHEN v_USE_EXT_ID THEN 0 ELSE 1 END, ',');
--@@End Implementation Overrid--
...
--@@Augment The Query To Add Display Emphasis And Initial Sort Order --
   OPEN p_CURSOR FOR
      SELECT TRANSACTION_ID,
         TRANSACTION_NAME,
         CONTRACT_NAME,
         CONTRACT_NUMBER,
         CASE WHEN TOTAL_COUNT = ALPHA_NULL_COUNT THEN NULL ELSE ALPHA_AMOUNT END "ALPHA_AMOUNT",
         CASE WHEN TOTAL_COUNT = BETA_NULL_COUNT THEN NULL ELSE BETA_AMOUNT END "BETA_AMOUNT",
         NVL(ALPHA_AMOUNT,0) - NVL(BETA_AMOUNT,0) "DIFFERENCE",
         ROUND(CASE WHEN NVL(ALPHA_AMOUNT,0) = 0 THEN CASE WHEN NVL(BETA_AMOUNT,0) = 0 THEN 0 ELSE -100 END ELSE (NVL(ALPHA_AMOUNT,0) - NVL(BETA_AMOUNT,0)) * 100 / ALPHA_AMOUNT END, 16) "PERCENTAGE",
         ACCEPT,
         CASE WHEN ALPHA_AMOUNT = BETA_AMOUNT THEN 1 ELSE 2 END "DELTA_DISPLAY_EMPHASIS"
      FROM
         (SELECT A.TRANSACTION_ID,
            ...
            SUM(CASE WHEN B.AMOUNT IS NULL THEN 1 ELSE 0 END) "ALPHA_NULL_COUNT",
            SUM(CASE WHEN C.AMOUNT IS NULL THEN 1 ELSE 0 END) "BETA_NULL_COUNT",
            COUNT(*) "TOTAL_COUNT",
         ...
         SERVICE_POINT G, SERVICE_POINT H, PURCHASING_SELLING_ENTITY I, PURCHASING_SELLING_ENTITY J, IT_STATUS K
         WHERE A.TRANSACTION_ID = D.TRANSACTION_ID
            ...
            AND CUT_DATE_BETWEEN(A.SCHEDULE_DATE, NVL(E.BEGIN_DATE, CONSTANTS.LOW_DATE), NVL(E.END_DATE, CONSTANTS.HIGH_DATE), v_TIME_ZONE) = 'Y'
            ...
            --AND (p_SCHEDULE_GROUP_ID = CONSTANTS.ALL_ID OR D.SCHEDULE_GROUP_ID = p_SCHEDULE_GROUP_ID)
      ...
      ORDER BY ABS(NVL(ALPHA_AMOUNT,0) - NVL(BETA_AMOUNT,0)) DESC, TRANSACTION_NAME;
...
--@@Begin Implementation Override --
--@@Augment The Query To Add Display Emphasis And Alter The Initial Sort Order --
   OPEN p_CURSOR FOR
      SELECT SUBSTR(FROM_CUT_AS_HED(SCHEDULE_DATE, p_TIME_ZONE, v_INTERVAL),1,10) "Date",
         SUBSTR(FROM_CUT_AS_HED(SCHEDULE_DATE, p_TIME_ZONE, v_INTERVAL),12) "Time",
         ALPHA_SCHEDULE_AMOUNT "Alpha",
         BETA_SCHEDULE_AMOUNT "Beta",
         NVL(ALPHA_SCHEDULE_AMOUNT,0) - NVL(BETA_SCHEDULE_AMOUNT,0) "Difference",
         ROUND(CASE WHEN NVL(ALPHA_SCHEDULE_AMOUNT,0) = 0 THEN CASE WHEN NVL(BETA_SCHEDULE_AMOUNT,0) = 0 THEN 0 ELSE -100 END ELSE (NVL(ALPHA_SCHEDULE_AMOUNT,0) - NVL(BETA_SCHEDULE_AMOUNT,0)) * 100 / ALPHA_SCHEDULE_AMOUNT END, 16) "Difference_Pct",
         CASE WHEN NVL(ALPHA_SCHEDULE_AMOUNT,0) = NVL(BETA_SCHEDULE_AMOUNT,0) THEN 1 ELSE 2 END "DELTA_DISPLAY_EMPHASIS"
      FROM
         (SELECT A.TRANSACTION_ID, A.TRANSACTION_NAME, C.SCHEDULE_DATE,
         MAX(CASE WHEN C.SCHEDULE_TYPE = v_ALPHA_SCHEDULE_TYPE AND C.SCHEDULE_STATE = v_ALPHA_SCHEDULE_STATE THEN C.AMOUNT ELSE NULL END) "ALPHA_SCHEDULE_AMOUNT",
         MAX(CASE WHEN C.SCHEDULE_TYPE = v_BETA_SCHEDULE_TYPE  AND C.SCHEDULE_STATE = v_BETA_SCHEDULE_STATE  THEN C.AMOUNT ELSE NULL END) "BETA_SCHEDULE_AMOUNT"
      FROM INTERCHANGE_TRANSACTION A,
         (SELECT CONTRACT_ID, BEGIN_DATE, NVL(END_DATE, CONSTANTS.HIGH_DATE) "END_DATE" FROM TP_CONTRACT_NUMBER WHERE CONTRACT_NAME = p_CONTRACT_NAME AND CONTRACT_NUMBER = p_CONTRACT_NUMBER AND BEGIN_DATE <= p_END_DATE AND NVL(END_DATE, CONSTANTS.HIGH_DATE) >= p_BEGIN_DATE) B,
         IT_SCHEDULE C
      WHERE A.TRANSACTION_ID = p_TRANSACTION_ID
         AND B.CONTRACT_ID(+) = A.CONTRACT_ID
         AND C.TRANSACTION_ID = A.TRANSACTION_ID
         ...
         AND CUT_DATE_BETWEEN(C.SCHEDULE_DATE, NVL(B.BEGIN_DATE, CONSTANTS.LOW_DATE), NVL(B.END_DATE, CONSTANTS.HIGH_DATE), p_TIME_ZONE) = 'Y'
         AND C.AS_OF_DATE = CONSTANTS.LOW_DATE
      GROUP BY A.TRANSACTION_ID, A.TRANSACTION_NAME, C.SCHEDULE_DATE)
      ORDER BY ABS(NVL(ALPHA_SCHEDULE_AMOUNT,0) - NVL(BETA_SCHEDULE_AMOUNT,0)) DESC, SCHEDULE_DATE;
--@@End Implementation Override --
