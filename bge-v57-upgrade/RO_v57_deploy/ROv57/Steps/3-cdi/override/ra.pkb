...
--@@Refactor Logic To Extend The Existing Time Perod Assignment --
...
SELECT * FROM AGGREGATE_ACCOUNT_ESP WHERE ACCOUNT_ID = p_ACCOUNT_ID AND ESP_ID = p_ESP_ID AND POOL_ID = p_POOL_ID ORDER BY BEGIN_DATE DESC;
...
BEGIN
	...
	p_STATUS := GA.SUCCESS;
	v_END_DATE := NULL_DATE(p_END_DATE);
   SELECT MAX(AGGREGATE_ID) INTO v_AGGREGATE_ID
   FROM AGGREGATE_ACCOUNT_ESP
   WHERE ACCOUNT_ID = p_ACCOUNT_ID
      AND ESP_ID = p_ESP_ID
      AND POOL_ID = p_POOL_ID;
--If The Key Combination Does Not Exist, Create A New Assignment Entry --
   IF v_AGGREGATE_ID IS NULL THEN
      v_AGGREGATE_ID := OID.NEXTVAL;
      INSERT INTO AGGREGATE_ACCOUNT_ESP(ACCOUNT_ID, ESP_ID, POOL_ID, BEGIN_DATE, END_DATE, AGGREGATE_ID, ENTRY_DATE)
      VALUES(p_ACCOUNT_ID, p_ESP_ID, p_POOL_ID, TRUNC(p_BEGIN_DATE), TRUNC(v_END_DATE), v_AGGREGATE_ID, SYSDATE);
   ELSE
-- Update The Current Account Esp Assignment--
      UPDATE AGGREGATE_ACCOUNT_ESP SET
         BEGIN_DATE = LEAST(p_BEGIN_DATE, BEGIN_DATE),
         END_DATE = GREATEST(v_END_DATE, END_DATE),
         ENTRY_DATE = SYSDATE
      WHERE ACCOUNT_ID = p_ACCOUNT_ID
         AND ESP_ID = p_ESP_ID
         AND POOL_ID = p_POOL_ID
         AND AGGREGATE_ID = v_AGGREGATE_ID;
--Make Sure There Are No Overlaps With Existing Entries --
      v_INITIAL := TRUE;
      FOR v_AGGREGATE_ACCOUNT_ESP IN c_AGGREGATE_ACCOUNT_ESP LOOP
         ...
         ELSIF v_AGGREGATE_ACCOUNT_ESP.BEGIN_DATE < v_PREV_END_DATE THEN
            DELETE AGGREGATE_ACCOUNT_ESP WHERE
      ...
      END LOOP;
   END IF;
--@@Trap Exception And Provide Feedback--
EXCEPTION
   WHEN OTHERS THEN
      ERRS.LOG_AND_RAISE('PUT_AGGREGATE_ACCOUNT_ESP Exception. Account Id: ' || TO_CHAR(p_ACCOUNT_ID) || ', ESP Id: ' || TO_CHAR(p_ESP_ID) || ', Pool Id: ' || TO_CHAR(p_POOL_ID) || ', Begin Date: ' || TO_CHAR(TRUNC(p_BEGIN_DATE),'MM/DD/YYYY') || ', End Date: ' || TO_CHAR(TRUNC(v_END_DATE),'MM/DD/YYYY') || ', Aggregate Id: ' || TO_CHAR(v_AGGREGATE_ID));
