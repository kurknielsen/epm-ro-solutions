...
c_ENABLE_PROXY_DAY_EXCLUSTION CONSTANT BOOLEAN := TRUE;
...
FUNCTION HAS_PROXY_DAY_ALL_EXCLUSION
   (
   p_SERVICE_DATE IN DATE
   ) RETURN BOOLEAN IS
v_COUNT PLS_INTEGER := 0;
BEGIN
   ...
   FROM PROXY_DAY_EXCLUSION PDE
   WHERE PDE.ACCOUNT_ID = 0
      AND p_SERVICE_DATE BETWEEN PDE.BEGIN_DATE AND NVL(PDE.END_DATE,PDE.BEGIN_DATE);
   RETURN v_COUNT > 0; 
END HAS_PROXY_DAY_ALL_EXCLUSION;

FUNCTION HAS_PROXY_DAY_EXCLUSION
   (
   p_ACCOUNT_SERVICE_ID IN NUMBER,
   p_SERVICE_DATE IN DATE
   ) RETURN BOOLEAN IS
...
BEGIN
   SELECT COUNT(1) INTO v_COUNT
   FROM ACCOUNT_SERVICE ASVC, PROXY_DAY_EXCLUSION PDE
   WHERE ASVC.ACCOUNT_SERVICE_ID = p_ACCOUNT_SERVICE_ID
      AND PDE.ACCOUNT_ID = ASVC.ACCOUNT_ID 
      AND p_SERVICE_DATE BETWEEN PDE.BEGIN_DATE AND NVL(PDE.END_DATE,PDE.BEGIN_DATE);
   RETURN v_COUNT > 0; 
END HAS_PROXY_DAY_EXCLUSION;
...
--@@Begin Implementation Override--
      IF c_ENABLE_PROXY_DAY_EXCLUSTION THEN
         IF HAS_PROXY_DAY_ALL_EXCLUSION(v_CANDIDATE.CANDIDATE_DAY) THEN
            IF LOGS.IS_DEBUG_ENABLED THEN
               LOGS.LOG_DEBUG('Discarded ' || TEXT_UTIL.TO_CHAR_DATE(v_CANDIDATE.CANDIDATE_DAY) || ' For ' || TEXT_UTIL.TO_CHAR_ENTITY(v_CANDIDATE.PROXY_DAY_METHOD_ID, EC.ED_PROXY_DAY_METHOD, TRUE) || ' Due To Proxy Day Exclusion For All Accounts.');
            END IF;
         ...
         ELSE
            ...
            IF v_COUNT > v_CANDIDATE.LOOKUP_CANDIDATE_LIMIT THEN
               IF LOGS.IS_DEBUG_ENABLED THEN
                  LOGS.LOG_DEBUG('Discarded ' || TEXT_UTIL.TO_CHAR_DATE(v_CANDIDATE.CANDIDATE_DAY) || ' For ' || TEXT_UTIL.TO_CHAR_ENTITY(v_CANDIDATE.PROXY_DAY_METHOD_ID, EC.ED_PROXY_DAY_METHOD, TRUE) || ' To Make The Candidate Limit.');
               END IF;
               DELETE PROXY_DAY_METHOD_WORK WHERE CURRENT OF cur_CANDIDATE_RETAIN;
            ELSE
               IF LOGS.IS_DEBUG_ENABLED THEN
                  LOGS.LOG_DEBUG('Selected ' || TEXT_UTIL.TO_CHAR_DATE(v_CANDIDATE.CANDIDATE_DAY) || ' For ' || TEXT_UTIL.TO_CHAR_ENTITY(v_CANDIDATE.PROXY_DAY_METHOD_ID, EC.ED_PROXY_DAY_METHOD, TRUE) || ' Ranked ' || TO_CHAR(v_COUNT) || '.');
               END IF;  
            END IF;
         END IF;
--@@End Implementation Override--
...
--@@Begin Implementation Override--
	FOR v_CANDIDATE IN cur_CANDIDATES LOOP
      IF c_ENABLE_PROXY_DAY_EXCLUSTION THEN
--@@Proxy Day Exclusion Logic--
         IF HAS_DR_EVENT(p_ACCOUNT_SERVICE_ID, v_CANDIDATE.CANDIDATE_DAY) THEN
            LOGS.LOG_DEBUG('Proxy Day ' || TEXT_UTIL.TO_CHAR_DATE(v_CANDIDATE.CANDIDATE_DAY) || ' was discarded for '|| GET_ACCOUNT_SERVICE_LOG_STR(p_ACCOUNT_SERVICE_ID) || ' because the account has a DR event for that day.');
         ELSIF HAS_PROXY_DAY_EXCLUSION(p_ACCOUNT_SERVICE_ID, v_CANDIDATE.CANDIDATE_DAY) THEN
            LOGS.LOG_DEBUG('Proxy Day ' || TEXT_UTIL.TO_CHAR_DATE(v_CANDIDATE.CANDIDATE_DAY) || ' was discarded for ' || GET_ACCOUNT_SERVICE_LOG_STR(p_ACCOUNT_SERVICE_ID)|| ' because the account has a proxy day exclusion for that day.');
         ELSE
            v_CANDIDATE_DATES.EXTEND();
            v_CANDIDATE_DATES(v_CANDIDATE_DATES.COUNT) := v_CANDIDATE.CANDIDATE_DAY;
         END IF; 
      ELSE  
--@@Standard Proxy Day Logic--
