...
BEGIN
	...
	IF p_CAST_CONTEXT.REQUEST_MODE = GA.STANDARD_MODE THEN
		...
		ELSIF p_CAST_CONTEXT.SERVICE_CODE = GA.ACTUAL_SERVICE THEN
			...
			CACHE_SCHEDULE_GROUP(p_IDS, p_CAST_CONTEXT.BEGIN_DATE, p_CAST_CONTEXT.END_DATE);
			CACHE_USAGE_FACTOR(p_IDS, p_CAST_CONTEXT.USAGE_FACTOR_CASE_ID, p_CAST_CONTEXT.BEGIN_DATE, p_CAST_CONTEXT.END_DATE);
--			CACHE_POOL(p_CAST_CONTEXT.BEGIN_DATE, p_CAST_CONTEXT.END_DATE); --@@Implementation Override--
			CACHE_IS_AGGREGATE_POOL;
         ...
         AND A.IS_SUB_AGGREGATE = 0
			...
			AND (EXISTS (SELECT 1 FROM ACCOUNT_CALENDAR AC
				WHERE AC.ACCOUNT_ID = A.ACCOUNT_ID
					...
					AND (CALENDAR_TYPE = ACCOUNTS_METERS.c_CALENDAR_TYPE_FORECAST OR (p_CAST_CONTEXT.REQUEST_TYPE = GA.BACKCAST_SERVICE AND CALENDAR_TYPE = ACCOUNTS_METERS.c_CALENDAR_TYPE_BACKCAST)))
--@@Begin Implementation Override--
--@@For Backcast Include Interval Accounts That Have Base Actuals Even If No Profile Is Assigned --
               OR (p_CAST_CONTEXT.REQUEST_TYPE = GA.BACKCAST_SERVICE
                  AND EXISTS(SELECT NULL FROM ACCOUNT_SERVICE AA, SERVICE BB, SERVICE_STATE CC
                         WHERE AA.ACCOUNT_ID = A.ACCOUNT_ID
                           AND BB.MODEL_ID = p_CAST_CONTEXT.MODEL_ID
                           AND BB.SCENARIO_ID = GA.BASE_SCENARIO_ID
                           AND BB.AS_OF_DATE = CONSTANTS.LOW_DATE
                           AND BB.ACCOUNT_SERVICE_ID = AA.ACCOUNT_SERVICE_ID
                           AND BB.SERVICE_ID = CC.SERVICE_ID
                           AND CC.SERVICE_CODE = GA.ACTUAL_SERVICE
                           AND CC.SERVICE_DATE BETWEEN p_CAST_CONTEXT.BEGIN_DATE AND p_CAST_CONTEXT.END_DATE)))
