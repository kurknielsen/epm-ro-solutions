...
c_NO_USAGE_FACTOR              CONSTANT NUMBER(8,6)  := 1;
c_INVALID_USAGE_FACTOR         CONSTANT NUMBER(8,6)  := -1;
...
v_MARK_TIME PLS_INTEGER := DBMS_UTILITY.GET_TIME;
BEGIN
   INSERT INTO CDI_ENERGY_INVALID_DATA(BILL_ACCOUNT, SERVICE_POINT, BEGIN_DATE, END_DATE, BILLED_USAGE, BILLED_KW, READ_CODE, BGE_TIMESTAMP, PROCESS_CODE, ERROR_MESSAGE, PROCESS_DATE, OSUSER, BGE_BEGIN_DATE, BGE_END_DATE)
   ...
   SELECT A.BILL_ACCOUNT, A.SERVICE_POINT, A.BEGIN_DATE, A.END_DATE, A.USAGE_KWH "BILLED_USAGE", A.DEMAND_KW "BILLED_KW", A.BILL_CODE "READ_CODE", A.REC_TS "BGE_TIMESTAMP", 'ERROR' "PROCESS_CODE",
      RTRIM(SUBSTR(
      CASE WHEN A.BEGIN_DATE > A.END_DATE     THEN 'Begin Date Is Less Than End Date, '   END ||
...
v_MARK_TIME PLS_INTEGER := DBMS_UTILITY.GET_TIME;
BEGIN
   INSERT INTO CDI_ENERGY_STAGING(BILL_ACCOUNT, SERVICE_POINT, BEGIN_DATE, END_DATE, BGE_BEGIN_DATE, BGE_END_DATE, BILLED_USAGE, BILLED_KW, READ_CODE, BGE_TIMESTAMP, PROCESS_CODE, ERROR_MESSAGE, PROCESS_DATE)
   WITH SERVICE_PERIOD AS
      (SELECT A.BILL_ACCOUNT, A.SERVICE_POINT, A.IDR_STATUS, MIN(A.EFFECTIVE_DATE) "EFFECTIVE_DATE", MAX(A.TERMINATION_DATE) "TERMINATION_DATE"
      FROM BGE_MASTER_ACCOUNT A
         JOIN CDI_USAGE_STAGING B ON B.BILL_ACCOUNT = A.BILL_ACCOUNT AND B.SERVICE_POINT = A.SERVICE_POINT
...
--@@Backup The End Date To The Prior Day But No Earlier Than The Begin Date --       
   SELECT A.BILL_ACCOUNT, A.SERVICE_POINT, A.BEGIN_DATE, GREATEST(A.END_DATE-1,A.BEGIN_DATE), A.BGE_BEGIN_DATE, A.BGE_END_DATE, A.USAGE_KWH "BILLED_USAGE", A.DEMAND_KW "BILLED_KW", A.BILL_CODE "READ_CODE", A.REC_TS "BGE_TIMESTAMP", 'VALID' "PROCESS_CODE", 'NEW' "ERROR_MESSAGE", SYSDATE
   FROM CDI_USAGE_STAGING A
      LEFT JOIN SERVICE_PERIOD B ON B.BILL_ACCOUNT = A.BILL_ACCOUNT AND B.SERVICE_POINT = A.SERVICE_POINT AND A.BEGIN_DATE BETWEEN B.EFFECTIVE_DATE AND B.TERMINATION_DATE AND A.END_DATE-CASE WHEN A.BEGIN_DATE = A.END_DATE THEN 0 ELSE 1 END BETWEEN B.EFFECTIVE_DATE AND B.TERMINATION_DATE
   WHERE
      CASE WHEN A.BEGIN_DATE > A.END_DATE     THEN 1 ELSE 0 END +
...
BEGIN
   LOGS.LOG_INFO('Cache Calendar Profile');
   ...
   FOR v_SELECT IN c_SELECT LOOP
      ...
      LOGS.LOG_INFO_DETAIL('Calendar Name: ' || v_CALENDAR_NAME || ', Calendar Id: ' || TO_CHAR(v_SELECT.CALENDAR_ID) || ', Begin Date: ' || TO_CHAR(v_SELECT.BEGIN_DATE, c_DATE_FORMAT) || ', End DATE: ' || TO_CHAR(v_SELECT.END_DATE, c_DATE_FORMAT) || ', Count: ' || TO_CHAR(v_SELECT.ENTRY_COUNT));
   ...
   LOGS.LOG_INFO('Cache Calendar Profile Complete. Records Posted: ' || TO_CHAR(v_CALENDAR_PROFILE_VALUE.COUNT) || ', Total Elapsed Seconds: ' || TO_CHAR(ROUND((DBMS_UTILITY.GET_TIME-v_MARK_TIME)/100)));
...
-- Capture The Profiled Usage Associated With Each Billed Usage Record -- --
   UPDATE CDI_MONTHLY_USAGE_FACTOR X SET USAGE_DAYS = ((END_DATE-BEGIN_DATE)+1), (PROFILED_USAGE, PROFILED_DAYS) = (SELECT SUM(PROFILE_VALUE), COUNT(*) FROM CALENDAR_PROFILE_VALUE WHERE CALENDAR_ID = X.CALENDAR_ID AND PROFILE_DATE BETWEEN X.BEGIN_DATE AND X.END_DATE);
...
-- Store The Usage Factor Back To The Monthly Usage Table --
   ...
   USING (SELECT BILL_ACCOUNT, SERVICE_POINT, BEGIN_DATE, END_DATE, BGE_TIMESTAMP, PROFILED_USAGE, USAGE_FACTOR FROM CDI_MONTHLY_USAGE_FACTOR) S
   ...
   WHEN MATCHED THEN
      UPDATE SET T.PROFILED_USAGE = S.PROFILED_USAGE, T.USAGE_FACTOR = S.USAGE_FACTOR, UF_PROCESS_ID = v_UF_PROCESS_ID; 
...
BEGIN
   ...
   IF v_COUNT > 0 THEN
      ...
      FROM CDI_ENERGY_STAGING A
         JOIN BGE_RTO_MONTHLY_USAGE B ON B.BILL_ACCOUNT = A.BILL_ACCOUNT AND B.SERVICE_POINT = A.SERVICE_POINT AND A.BGE_TIMESTAMP >= B.BGE_TIMESTAMP AND B.BEGIN_DATE <= NVL(A.END_DATE, CONSTANTS.HIGH_DATE) AND NVL(B.END_DATE, CONSTANTS.HIGH_DATE)  >= A.BEGIN_DATE AND B.PROCESS_CODE  <> 'VALID' AND B.ERROR_MESSAGE <> 'NEW'
      ...
      LOGS.LOG_INFO('Number Of Records Posted To The Rowid Collector: ' || TO_CHAR(v_CI_ROWID.COUNT), v_PROCEDURE_NAME, 'Collect Invalid Record Row Ids', c_PACKAGE_NAME);
      ...
      INSERT INTO CDI_ENERGY_INVALID_DATA(BILL_ACCOUNT, SERVICE_POINT, BEGIN_DATE, END_DATE, BILLED_USAGE, BILLED_KW, READ_CODE, BGE_TIMESTAMP, PROCESS_CODE, ERROR_MESSAGE, PROCESS_DATE, BGE_BEGIN_DATE, BGE_END_DATE)
      SELECT BILL_ACCOUNT, SERVICE_POINT, BEGIN_DATE, END_DATE, BILLED_USAGE, BILLED_KW, READ_CODE, BGE_TIMESTAMP, 'CANCEL', 'READ CANCEL ROW', SYSDATE, BGE_BEGIN_DATE, BGE_END_DATE
   ...
   FOR v_INDEX IN v_CONTAINER.FIRST..v_CONTAINER.LAST LOOP
      ...
      IF v_TOKENS.COUNT = 9 THEN
         BEGIN
            ...
            v_USAGE_STAGING.TIME_PERIOD    := NVL(INITCAP(v_TOKENS(8)), c_ALL);
...
BEGIN
   LOAD_PERIOD_USAGE(p_BEGIN_DATE, p_END_DATE, p_STATUS, p_MESSAGE);
