...
c_ACCEPT_PLC_NSPL_TICKETS      CONSTANT VARCHAR2(64) := 'Accept PLC/NSPL Tickets';
c_CALCULATE_PLC_NSPL_TICKETS   CONSTANT VARCHAR2(64) := 'Calculate PLC/NSPL Tickets';
c_ASSIGN_DEFAULT_PLC_TAG       CONSTANT VARCHAR2(64) := 'Assign Default PLC Tag';
c_LIBRARY_NAME                 CONSTANT VARCHAR2(32) := 'ALM Impact Profiles';
c_MODULE_SETTLEMENT            CONSTANT VARCHAR2(32) := 'Settlement';
...
BEGIN
   IF NOT l_IS_CACHED THEN
      ...
      ASSERT(l_PLC_ID <> CONSTANTS.NOT_ASSIGNED, 'Ancillary Service Not Defined For: ' || c_NSPL_NAME);
      l_IS_CACHED := TRUE;
   END IF;
END CACHE_ANCILLARY_SERVICE_ID;

PROCEDURE POPULATE_TOU_USAGE_FACTOR(p_PEAK_DATE IN DATE, p_TEMPLATE_ID IN NUMBER, p_PERIOD_ID IN NUMBER) AS
BEGIN
   EXECUTE IMMEDIATE 'TRUNCATE TABLE CDI_TOU_USAGE_FACTORS_SHORT';
   INSERT INTO CDI_TOU_USAGE_FACTORS_SHORT(BILL_ACCOUNT, SERVICE_POINT, ACCOUNT_ID, ACCOUNT_EXTERNAL_IDENTIFIER, ESP_ID, ESP_NAME, POOL_ID, POOL_NAME, PROFILE_ID, PROFILE_NAME, TEMPLATE_ID, TEMPLATE_NAME, PERIOD_ID, PERIOD_NAME, BEGIN_DATE, END_DATE, BILLED_USAGE, PROFILED_USAGE, USAGE_FACTOR, CREATE_DTS, LAST_UPDATE_DTS)
   SELECT BILL_ACCOUNT, SERVICE_POINT, ACCOUNT_ID, ACCOUNT_EXTERNAL_IDENTIFIER, ESP_ID, ESP_NAME, POOL_ID, POOL_NAME, PROFILE_ID, PROFILE_NAME, TEMPLATE_ID, TEMPLATE_NAME, PERIOD_ID, PERIOD_NAME, BEGIN_DATE, END_DATE, BILLED_USAGE, PROFILED_USAGE, USAGE_FACTOR, CREATE_DTS, LAST_UPDATE_DTS
   FROM CDI_TOU_USAGE_FACTORS
   WHERE TRUNC(p_PEAK_DATE) BETWEEN BEGIN_DATE AND END_DATE
      AND TEMPLATE_ID IN (p_TEMPLATE_ID, l_ANYTIME_TEMPLATE_ID, l_ALL_TEMPLATE_ID)
      AND PERIOD_ID IN (p_PERIOD_ID, l_ANYTIME_PERIOD_ID, l_ALL_PERIOD_ID);
   COMMIT;
END POPULATE_TOU_USAGE_FACTOR;
...
BEGIN
   ...
   WITH CANDIDATES AS
      ...
      WHERE TRUNC(p_PEAK_DATE) BETWEEN EFFECTIVE_DATE AND NVL(TERMINATION_DATE, CONSTANTS.HIGH_DATE)
         AND IDR_STATUS = 'Y'
         AND STUDY_ID NOT IN (SELECT PROFILE_NAME FROM LOAD_PROFILE))
   SELECT /*+ ORDERED FULL(A) */
      A.BILL_ACCOUNT, A.SERVICE_POINT, A.PREMISE_NUMBER, A.IDR_STATUS, A.RATE_CLASS, A.VOLTAGE_LEVEL, B.PROFILE_ID, B.PROFILE_NAME, C.POINT_DATE, C.POINT_VAL, NULL, NULL, NULL, NULL, NULL, SYSDATE, SYSDATE, 1, NULL
   FROM BGE_MASTER_ACCOUNT    A
      JOIN LOAD_PROFILE       B ON B.PROFILE_NAME = A.STUDY_ID
      JOIN LOAD_PROFILE_POINT C ON C.PROFILE_ID = B.PROFILE_ID
   ...
   SELECT /*+ ORDERED FULL(A) */
      A.BILL_ACCOUNT, A.SERVICE_POINT, A.PREMISE_NUMBER, A.IDR_STATUS, A.RATE_CLASS, A.VOLTAGE_LEVEL, B.PROFILE_ID, B.PROFILE_NAME, C.POINT_DATE, C.POINT_VAL, NULL, NULL, NULL, NULL, NULL, SYSDATE, SYSDATE, 1, NULL
   FROM BGE_MASTER_ACCOUNT    A
      JOIN LOAD_PROFILE       B ON B.PROFILE_NAME = A.STUDY_ID AND B.PROFILE_NAME = 'P'
      JOIN LOAD_PROFILE_POINT C ON C.PROFILE_ID = B.PROFILE_ID
   WHERE A.BILL_ACCOUNT IN (SELECT BILL_ACCOUNT FROM CANDIDATES)
      AND p_PEAK_DATE BETWEEN A.EFFECTIVE_DATE AND NVL(A.TERMINATION_DATE, CONSTANTS.HIGH_DATE);
   COMMIT;
...
BEGIN
   ...
   SELECT /*+ ORDERED FULL(A) */
      A.BILL_ACCOUNT, A.SERVICE_POINT, A.PREMISE_NUMBER, A.IDR_STATUS, A.RATE_CLASS, A.VOLTAGE_LEVEL, B.PROFILE_ID, B.PROFILE_NAME, C.POINT_DATE, C.POINT_VAL, NULL, NULL, NULL, NULL, SYSDATE, SYSDATE, 1, NULL
   FROM BGE_MASTER_ACCOUNT A
      JOIN LOAD_PROFILE B ON B.PROFILE_NAME = A.STUDY_ID
...
PROCEDURE POPULATE_PLC_DETAIL(p_PEAK_DATE IN DATE, p_TEMPLATE_ID IN NUMBER, p_PERIOD_ID IN NUMBER) AS
BEGIN
  ...
  SELECT BILL_ACCOUNT, SERVICE_POINT, PREMISE_NUMBER, IDR_STATUS, RATE_CLASS, VOLTAGE_LEVEL, PROFILE_ID, PROFILE_NAME, POINT_DATE, POINT_VAL, NULL, NULL, NULL, BILLED_KW, SYSDATE, SYSDATE, NULL, NULL
  FROM
     (SELECT /*+ ORDERED PARALLEL(X,8) FULL(A) USE_HASH(X,A) USE_HASH(A,B) USE_HASH(B,C) USE_HASH(C,D) USE_HASH(D,E)  USE_HASH(E,F) */
        A.BILL_ACCOUNT, A.SERVICE_POINT, A.PREMISE_NUMBER, A.IDR_STATUS, A.RATE_CLASS, A.VOLTAGE_LEVEL, F.PROFILE_ID, F.PROFILE_NAME, L.POINT_DATE,L.POINT_VAL, X.BILLED_KW, ROW_NUMBER() OVER(PARTITION BY A.BILL_ACCOUNT, A.SERVICE_POINT, A.PREMISE_NUMBER, A.IDR_STATUS, A.RATE_CLASS, A.VOLTAGE_LEVEL, F.PROFILE_ID, F.PROFILE_NAME, L.POINT_DATE, L.POINT_VAL, X.BILLED_KW ORDER BY X.BILLED_KW) AS RN
      FROM BGE_RTO_MONTHLY_USAGE X
         ...
         JOIN ACCOUNT               B ON B.ACCOUNT_EXTERNAL_IDENTIFIER = A.RTO_ACCOUNT_ID
         JOIN ACCOUNT_CALENDAR      C ON C.CASE_ID = GA.BASE_CASE_ID AND C.ACCOUNT_ID = B.ACCOUNT_ID AND C.CALENDAR_TYPE = c_MODULE_SETTLEMENT AND TRUNC(p_PEAK_DATE) BETWEEN C.BEGIN_DATE AND NVL(C.END_DATE, CONSTANTS.HIGH_DATE)
         JOIN CALENDAR_PROFILE      D ON D.CALENDAR_ID = C.CALENDAR_ID
         JOIN CALENDAR              E ON E.CALENDAR_ID = C.CALENDAR_ID
         JOIN LOAD_PROFILE          F ON F.PROFILE_ID = D.PROFILE_ID
         JOIN LOAD_PROFILE_POINT    L ON L.PROFILE_ID = F.PROFILE_ID AND L.POINT_DATE = p_PEAK_DATE
      WHERE SUBSTR(TRIM(X.READ_CODE),2,1) <> 'X'
         ...
         AND X.TEMPLATE_ID IN (p_TEMPLATE_ID, l_ANYTIME_TEMPLATE_ID, l_ALL_TEMPLATE_ID)
         AND X.PERIOD_ID IN (p_PERIOD_ID, l_ANYTIME_PERIOD_ID, l_ALL_PERIOD_ID)
...
PROCEDURE POPULATE_NSPL_DETAIL(p_PEAK_DATE IN DATE, p_TEMPLATE_ID IN NUMBER, p_PERIOD_ID IN NUMBER) AS
BEGIN
   ...
   SELECT BILL_ACCOUNT, SERVICE_POINT, PREMISE_NUMBER, IDR_STATUS, RATE_CLASS, VOLTAGE_LEVEL, PROFILE_ID, PROFILE_NAME, POINT_DATE, POINT_VAL, NULL, NULL, NULL, NULL, BILLED_KW, SYSDATE, SYSDATE, NULL, NULL
   FROM
...
PROCEDURE UPDATE_PLC_USAGE_FACTOR(p_PEAK_HOUR IN DATE, p_CUT_PEAK IN DATE) AS
v_COUNT PLS_INTEGER := 0;
BEGIN
  SELECT COUNT(*) INTO v_COUNT FROM CDI_TOU_USAGE_FACTORS_SHORT;
  IF v_COUNT = 0 THEN
     LOGS.LOG_WARN('No Usage Factor For Updates');
  ELSE
     EXECUTE IMMEDIATE ('ALTER SESSION ENABLE PARALLEL DML');
     MERGE /*+ FULL(A) PARALLEL(A,8) */ INTO CDI_PLC_ICAP_DETAIL T
     USING
       ...
       GROUP BY BILL_ACCOUNT, SERVICE_POINT) S
     ON (T.BILL_ACCOUNT = S.BILL_ACCOUNT AND T.SERVICE_POINT = S.SERVICE_POINT AND T.PEAK_DATE = p_PEAK_HOUR)
     WHEN MATCHED THEN
        UPDATE SET T.USAGE_FACTOR = S.USAGE_FACTOR;
     v_COUNT := SQL%ROWCOUNT;
     COMMIT;
     EXECUTE IMMEDIATE ('ALTER SESSION DISABLE PARALLEL DML');
     LOGS.LOG_INFO('PLC Usage Factor Update On ' || TO_CHAR(SYSDATE,c_DATE_TIME_FORMAT)|| ', Count: ' || TO_CHAR(v_COUNT));
  END IF;
END UPDATE_PLC_USAGE_FACTOR;

PROCEDURE UPDATE_NSPL_USAGE_FACTOR(p_PEAK_HOUR IN DATE) AS
v_COUNT PLS_INTEGER;
BEGIN
  SELECT COUNT(*) INTO v_COUNT FROM CDI_TOU_USAGE_FACTORS_SHORT;
   IF v_COUNT = 0 THEN
   ...
   ELSE
      EXECUTE IMMEDIATE ('ALTER SESSION ENABLE PARALLEL DML');
      MERGE /*+ FULL(A) PARALLEL(A,8) */ INTO CDI_PLC_TX_DETAIL T
      USING
         (SELECT /*+ FULL(BB) PARALLEL(BB,8) USE_HASH(A,BB) */ BILL_ACCOUNT, SERVICE_POINT, AVG(USAGE_FACTOR) "USAGE_FACTOR"
         FROM  CDI_TOU_USAGE_FACTORS_SHORT BB WHERE TRUNC(p_PEAK_HOUR) BETWEEN BEGIN_DATE AND END_DATE
         GROUP BY BILL_ACCOUNT, SERVICE_POINT) S
      ON (T.BILL_ACCOUNT = S.BILL_ACCOUNT AND T.SERVICE_POINT = S.SERVICE_POINT AND T.PEAK_DATE = p_PEAK_HOUR)
      WHEN MATCHED THEN
      ...
      COMMIT;
      EXECUTE IMMEDIATE ('ALTER SESSION DISABLE PARALLEL DML');
      LOGS.LOG_INFO('NSPL Usage Factor Update On ' || TO_CHAR(SYSDATE,c_DATE_TIME_FORMAT)|| ', Count: ' || TO_CHAR(v_COUNT));
   END IF;
END UPDATE_NSPL_USAGE_FACTOR;
...
BEGIN
   ...
   ASSERT(v_PROFILE_LIBRARY_ID IS NOT NULL, 'Profile Libbrary "' || c_LIBRARY_NAME || '" Is Not Defined');
   FOR v_SELECT IN c_SELECT LOOP
      UPDATE CDI_PLC_TX_DETAIL
       ...
       WHERE BILL_ACCOUNT = v_SELECT.BILL_ACCOUNT
         ...
         IF MOD(v_COUNT, 5000) = 0 THEN
            COMMIT;
            LOGS.LOG_INFO(0,'ALM Updates I ' || TO_CHAR(SYSDATE, c_DATE_TIME_FORMAT) ||  ', Count:  ' || TO_CHAR(v_COUNT));
         END IF;
   END LOOP;
   COMMIT;
...
BEGIN
   ...
   FOR v_SELECT IN c_SELECT LOOP
      ...
      END IF;
   END LOOP;
   LOGS.LOG_INFO('ALM Updates Complete At ' || TO_CHAR(SYSDATE,c_DATE_TIME_FORMAT)|| ', Total Count: ' || TO_CHAR(v_COUNT));
   COMMIT;
...
BEGIN
   SELECT MAX(PLC_BEGIN_DATE), MAX(PLC_END_DATE) INTO v_BEGIN_DATE, v_END_DATE
   FROM CDI_POLR_TO_PLC_MAP
   WHERE PLC_BEGIN_DATE BETWEEN (p_PLAN_YEAR + 365) AND (p_PLAN_YEAR + 700);
...
--   GROUP BY BILL_ACCOUNT, SERVICE_POINT, PREMISE_NUMBER;
   ...
   FROM
      (
      SELECT BILL_ACCOUNT, SERVICE_POINT, PREMISE_NUMBER, HOURLY_PLC FROM CDI_PLC_ICAP_DETAIL WHERE HOURLY_PLC IS NOT NULL
      UNION ALL
      SELECT BILL_ACCOUNT, SERVICE_POINT, PREMISE_NUMBER, HOURLY_PLC FROM CDI_PLC_ICAP_INT_DETAIL WHERE HOURLY_PLC IS NOT NULL
      )   
   GROUP BY BILL_ACCOUNT, SERVICE_POINT, PREMISE_NUMBER;
   COMMIT;
...
BEGIN
   ...
   FROM
      (
      SELECT BILL_ACCOUNT, SERVICE_POINT, PREMISE_NUMBER, HOURLY_PLC FROM CDI_PLC_TX_DETAIL WHERE HOURLY_PLC IS NOT NULL
      UNION ALL
      SELECT BILL_ACCOUNT, SERVICE_POINT, PREMISE_NUMBER, HOURLY_PLC FROM CDI_PLC_TX_INT_DETAIL WHERE HOURLY_PLC IS NOT NULL
      )   
   GROUP BY BILL_ACCOUNT, SERVICE_POINT, PREMISE_NUMBER;
--   SELECT BILL_ACCOUNT, SERVICE_POINT, PREMISE_NUMBER, v_TAG, v_BEGIN_DATE, v_END_DATE, AVG(HOURLY_PLC)
--   FROM CDI_PLC_TX_DETAIL
...
PROCEDURE PREPROCESSOR_PLC(p_PLAN_YEAR IN DATE) AS
v_PROCEDURE_NAME VARCHAR2(32) := 'PREPROCESSOR_PLC'; 
v_ANCILLARY_SERVICE_ID NUMBER(9);
v_MIN_DATE     DATE;
v_MAX_DATE     DATE;
v_TEMPLATE_ID  NUMBER(9);
v_PERIOD_ID    NUMBER(9);
v_BEGIN_DATE   DATE := TO_DATE(TO_CHAR(p_PLAN_YEAR,'YYYY')||'0101','YYYYMMDD');
v_END_DATE     DATE := TO_DATE(TO_CHAR(p_PLAN_YEAR,'YYYY')||'1231','YYYYMMDD');
CURSOR c_SELECT IS
...
BEGIN
   ...
   WHERE ANCILLARY_SERVICE_ID = l_PLC_ID
      AND PEAK_DATE BETWEEN v_BEGIN_DATE AND END_DATE;
   ASSERT(v_MIN_DATE IS NOT NULL, 'PLC Peak Dates Not Found For Plan Year: ' || TO_CHAR(p_PLAN_YEAR,'YYYY'));
   FOR v_SELECT IN c_SELECT LOOP
      LOGS.LOG_INFO('Populate CDI_PLC_ICAP_DETAIL For ' || TO_CHAR(v_SELECT.PEAK_HOUR, c_DATE_TIME_FORMAT), v_PROCEDURE_NAME, 'Preprocessor:POPULATE_PLC_DETAIL', c_PACKAGE_NAME);
      POPULATE_PLC_DETAIL(v_SELECT.PEAK_HOUR, v_TEMPLATE_ID, v_PERIOD_ID);
      LOGS.LOG_INFO('Populate CDI_TOU_USAGE_FACTORS_SHORT Table', v_PROCEDURE_NAME, 'Preprocessor:POPULATE_SHORT_CDI_TOU_UF', c_PACKAGE_NAME);
      POPULATE_TOU_USAGE_FACTOR(v_SELECT.PEAK_HOUR, v_TEMPLATE_ID, v_PERIOD_ID);
      LOGS.LOG_INFO('Update CDI_PLC_ICAP_DETAIL Usage Factor', v_PROCEDURE_NAME, 'Preprocessor:UPDATE_ICAP_USAGE_FACTOR', c_PACKAGE_NAME);
      UPDATE_PLC_USAGE_FACTOR(v_SELECT.PEAK_HOUR, v_SELECT.CUT_PEAK);
      LOGS.LOG_INFO('Populate CDI_PLC_ICAP_INT_DETAIL For Peak Hour: ' || TO_CHAR(v_SELECT.PEAK_HOUR, c_DATE_TIME_FORMAT), v_PROCEDURE_NAME, 'Preprocessor:Populate CDI_PLC_ICAP_INT_DETAIL For Peak Hour: ' || TO_CHAR(v_SELECT.PEAK_HOUR, c_DATE_TIME_FORMAT), c_PACKAGE_NAME);
      POPULATE_PLC_INTERVAL_DETAIL(v_SELECT.PEAK_HOUR);
      LOGS.LOG_INFO('Update CDI_PLC_ICAP_INT_DETAIL For ALM AddBacks', v_PROCEDURE_NAME, 'Preprocessor:POPULATE_CDI_PLC_ICAP_INT', c_PACKAGE_NAME);
      UPDATE_ALM_PLC_INTERVAL(v_SELECT.PEAK_HOUR);
   END LOOP;
   LOGS.LOG_INFO('Update CDI_PLC_ICAP_DETAIL For Loss Factor', v_PROCEDURE_NAME, 'Preprocessor:UPDATE_ICAP_LOSS_FACTOR', c_PACKAGE_NAME);
   UPDATE_PLC_LOSS_FACTOR;
   LOGS.LOG_INFO('Update CDI_PLC_ICAP_INT_DETAIL For Loss Factor', v_PROCEDURE_NAME, 'Preprocessor:UPDATE_INT_LOSS_FACTOR', c_PACKAGE_NAME);
   UPDATE_INTERVAL_LOSS_FACTOR;
END PREPROCESSOR_PLC;

PROCEDURE PREPROCESSOR_NSPL(p_PLAN_YEAR IN DATE) AS
...
CURSOR c_SELECT_PEAK_HOUR(p_BEGIN_DATE IN DATE, p_END_DATE IN DATE) IS
   ...
   ORDER BY PEAK_DATE;
v_PROCEDURE_NAME VARCHAR2(32) := 'PREPROCESSOR_TX';
v_CURRENT_DATE DATE;
v_ANCILLARY_SERVICE_ID NUMBER(9);
v_MIN_DATE    DATE := CONSTANTS.HIGH_DATE;
v_MAX_DATE    DATE := CONSTANTS.LOW_DATE;
v_END_DATE    DATE := TO_DATE(TO_CHAR(ADD_MONTHS(p_PLAN_YEAR,12),'YYYY')||'0101','YYYYMMDD');
v_TEMPLATE_ID NUMBER(9);
v_PERIOD_ID   NUMBER(9);
v_COUNT       PLS_INTEGER;
...
v_ALM_DAYS t_ALM_DAYS_TBL;
  ...
  END IS_ALM_DAY;
BEGIN
   LOGS.LOG_INFO('Get NSPL Peak Dates From The ANCILLARY_SERVICE_AREA_PEAK Table For Plan Year: ' || TO_CHAR(p_PLAN_YEAR, c_DATE_FORMAT), v_PROCEDURE_NAME, 'Preprocessor NSPL: Get Min And Max Peak Dates', c_PACKAGE_NAME);
   SELECT TRUNC(MIN(PEAK_DATE)), TRUNC(MAX(PEAK_DATE)) INTO v_MIN_DATE, v_MAX_DATE FROM ANCILLARY_SERVICE_AREA_PEAK WHERE ANCILLARY_SERVICE_ID = l_NSPL_ID AND PEAK_DATE BETWEEN p_PLAN_YEAR AND v_END_DATE;
   ASSERT(v_MIN_DATE IS NOT NULL, 'No NSPL Peak Dates Defined In The ANCILLARY_SERVICE_AREA_PEAK Table For Plan Year: ' || TO_CHAR(p_PLAN_YEAR, c_DATE_FORMAT));
   LOGS.LOG_INFO('Get All Possible ALM Days', v_PROCEDURE_NAME, 'Preprocessor', c_PACKAGE_NAME); 
   FOR v_SELECT IN c_SELECT_LOAD_DATE(v_MIN_DATE, v_MAX_DATE) LOOP
      v_ALM_DAYS(TO_CHAR(v_SELECT.LOAD_DATE,'YYYYMMDDHH24')) := 'Y';
   END LOOP;
   FOR v_SELECT IN c_SELECT_PEAK_HOUR(p_PLAN_YEAR, v_END_DATE) LOOP
      LOGS.LOG_INFO('Populate CDI_PLC_TX_DETAIL Table', v_PROCEDURE_NAME, 'Preprocessor:POPULATE_NSPL_DETAIL', c_PACKAGE_NAME); 
      POPULATE_NSPL_DETAIL(v_SELECT.PEAK_HOUR, v_TEMPLATE_ID, v_PERIOD_ID);
      SELECT COUNT(*) INTO v_COUNT FROM CDI_PLC_TX_DETAIL;
      IF v_COUNT = 0 THEN
         LOGS.LOG_INFO('CDI_PLC_TX_DETAIL Table Is Empty', v_PROCEDURE_NAME, 'Preprocessor', c_PACKAGE_NAME); 
      ELSE
         LOGS.LOG_INFO('Populate NSPL Content To The CDI_TOU_USAGE_FACTORS_SHORT Table', v_PROCEDURE_NAME, 'Preprocessor:POPULATE_TOU_USAGE_FACTOR', c_PACKAGE_NAME); 
         POPULATE_TOU_USAGE_FACTOR(v_SELECT.PEAK_HOUR, v_TEMPLATE_ID, v_PERIOD_ID);
         LOGS.LOG_INFO('Update CDI_PLC_TX_DETAIL For NSPL Usage Factor', v_PROCEDURE_NAME, 'Preprocessor:UPDATE_NSPL_LOSS_FACTOR', c_PACKAGE_NAME); 
         UPDATE_NSPL_LOSS_FACTOR;
         LOGS.LOG_INFO('Populate NSPL Content To The CDI_PLC_TX_INT_DETAIL Table For Peak Hour: ' || TO_CHAR(v_SELECT.PEAK_HOUR, c_DATE_TIME_FORMAT), v_PROCEDURE_NAME, 'Preprocessor:POPULATE_CDI_PLC_TX_INT', c_PACKAGE_NAME); 
         POPULATE_NSPL_INTERVAL_DETAIL(v_SELECT.PEAK_HOUR);
         IF IS_ALM_DAY(v_SELECT.PEAK_HOUR) THEN
            LOGS.LOG_INFO('Update ALM Monthly', v_PROCEDURE_NAME, 'Preprocessor:UPDATE_ALM_MONTHLY_VALUE', c_PACKAGE_NAME); 
            UPDATE_ALM_MONTHLY_VALUE(v_SELECT.PEAK_HOUR);
            LOGS.LOG_INFO('Update ALM Interval', v_PROCEDURE_NAME, 'Preprocessor:UPDATE_ALM_INTERVAL_VALUE', c_PACKAGE_NAME); 
            UPDATE_ALM_INTERVAL_VALUE(v_SELECT.PEAK_HOUR);
         END IF;
      END IF;
   END LOOP;
   LOGS.LOG_INFO('Update CDI_PLC_TX_DETAIL For Loss Factor', v_PROCEDURE_NAME, 'Preprocessor:UPDATE_NSPL_LOSS_FACTOR', c_PACKAGE_NAME); 
   UPDATE_NSPL_LOSS_FACTOR;
   LOGS.LOG_INFO('Update CDI_PLC_TX_INT_DETAIL For Loss Factor', v_PROCEDURE_NAME, 'Preprocessor:UPDATE_NSPL_INT_LOSS_FACTOR', c_PACKAGE_NAME); 
   UPDATE_NSPL_INT_LOSS_FACTOR;
END PREPROCESSOR_NSPL;
...
v_PLAN_YEAR DATE := TO_DATE(p_PLAN_YEAR, c_DATE_FORMAT);
...
-- Start The Process Log --
   ...
   LOGS.LOG_INFO('Plan Year: ' || TO_CHAR(v_PLAN_YEAR, c_DATE_FORMAT));
   CACHE_ANCILLARY_SERVICE_ID;
   IF p_ANCILLARY_SERVICE_ID = l_PLC_ID THEN
      TRUNCATE_PLC_TABLES;
      PREPROCESSOR_PLC(v_PLAN_YEAR);
   ELSE
      TRUNCATE_NSPL_TABLES;
      PREPROCESSOR_NSPL(v_PLAN_YEAR);
   END IF;
...
PROCEDURE SUBMIT_BACKGROUND_JOB(p_TASK_NAME IN VARCHAR2, p_ANCILLARY_SERVICE_ID IN NUMBER, p_PLAN_YEAR IN VARCHAR2) AS
BEGIN
NULL;
...
-- Start The Process Log --
   ...
   CACHE_ANCILLARY_SERVICE_ID;
   IF p_ANCILLARY_SERVICE_ID = l_PLC_ID THEN
      LOGS.LOG_INFO('Generate PLC Buildups For Plan Year:' || p_PLAN_YEAR, v_PROCEDURE_NAME, 'Begin:GENERATE_PLC_BUILDUPS', c_PACKAGE_NAME); 
      GENERATE_PLC_BUILDUPS(v_PLAN_YEAR);
      LOGS.LOG_INFO('Calculate PLC Scale Factors', v_PROCEDURE_NAME, 'Begin:CALCULATE_PLC_SCALE_FACTOR', c_PACKAGE_NAME); 
      CALCULATE_PLC_SCALE_FACTOR;
      LOGS.LOG_INFO('Calculate PLC Obligation Factors', v_PROCEDURE_NAME, 'Begin:CALCULATE_PLC_OBLIGATION_FAC', c_PACKAGE_NAME); 
      CALCULATE_PLC_OBLIGATION_FAC;
      LOGS.LOG_INFO('Update PLC Details', v_PROCEDURE_NAME, 'Begin:UPDATE_PLC_DETAIL', c_PACKAGE_NAME); 
      UPDATE_PLC_DETAIL;
      LOGS.LOG_INFO('Update Interval PLC Details', v_PROCEDURE_NAME, 'Begin:UPDATE_PLC_INTERVAL_DETAIL', c_PACKAGE_NAME); 
      UPDATE_PLC_INTERVAL_DETAIL;
      LOGS.LOG_INFO('Update New Interval PLC Details', v_PROCEDURE_NAME, 'Begin:UPDATE_PLC_INTERVAL_DTL_NEW', c_PACKAGE_NAME); 
      UPDATE_PLC_INTERVAL_DTL_NEW;
      LOGS.LOG_INFO('Generate PLC Tickets', v_PROCEDURE_NAME, 'Begin:GENERATE_PLC_TICKETS', c_PACKAGE_NAME); 
      GENERATE_PLC_TICKETS(v_PLAN_YEAR);
   ELSE
      LOGS.LOG_INFO('Generate NSPL Buildups For Plan Year:' || p_PLAN_YEAR, v_PROCEDURE_NAME, 'Begin:GENERATE_TX_BUILDUPS', c_PACKAGE_NAME); 
      GENERATE_NSPL_BUILDUPS(v_PLAN_YEAR);
      LOGS.LOG_INFO('Calculate NSPL Scale Factors', v_PROCEDURE_NAME, 'Begin:CALCULATE_NSPL_SCALE_FACTOR', c_PACKAGE_NAME); 
      CALCULATE_NSPL_SCALE_FACTOR;
      LOGS.LOG_INFO('Calculate NSPL Obligation Factors', v_PROCEDURE_NAME, 'Begin:CALCULATE_NSPL_OBLIGATION_FAC', c_PACKAGE_NAME); 
      CALCULATE_NSPL_OBLIGATION_FAC;
      LOGS.LOG_INFO('Update NSPL Details', v_PROCEDURE_NAME, 'Begin:UPDATE_NSPL_DETAIL', c_PACKAGE_NAME); 
      UPDATE_NSPL_DETAIL;
      LOGS.LOG_INFO('Update Interval NSPL Details', v_PROCEDURE_NAME, 'Begin:UPDATE_NSPL_INTERVAL_DETAIL', c_PACKAGE_NAME); 
      UPDATE_NSPL_INTERVAL_DETAIL;
      LOGS.LOG_INFO('Update New Interval NSPL Details', v_PROCEDURE_NAME, 'Begin:UPDATE_NSPL_INTERVAL_DTL_NEW', c_PACKAGE_NAME); 
      UPDATE_NSPL_INTERVAL_DTL_NEW;
      LOGS.LOG_INFO('Generate NSPL Tickets', v_PROCEDURE_NAME, 'Begin:GENERATE_NSPL_TICKETS', c_PACKAGE_NAME); 
...
--         p_MESSAGE := 'Inappropriate selection criteria.' || k_CRLF || 'If ''Anytime'' is selected, it must be specified for both filters.';
--      END IF;
--
--      OPEN p_CURSOR FOR
--         SELECT BILL_ACCOUNT,
...
BEGIN
   CACHE_ANCILLARY_SERVICE_ID;
   v_BEGIN_DATE := TO_DATE(CASE WHEN p_ANCILLARY_SERVICE_ID = l_PLC_ID THEN '6/1/' ELSE '1/1/' END || p_PLAN_YEAR, c_DATE_FORMAT);
   OPEN p_CURSOR FOR
      SELECT ANCILLARY_SERVICE_ID, AREA_ID, BEGIN_DATE, END_DATE, TRUNC(FROM_CUT(PEAK_DATE,GA.LOCAL_TIME_ZONE)) "PEAK_DAY", TO_CHAR(FROM_CUT(PEAK_DATE, GA.LOCAL_TIME_ZONE),'HH24') "PEAK_HOUR", PEAK_VAL "PEAK_VALUE", ROWID "ENTRY_ROWID"
      FROM ANCILLARY_SERVICE_AREA_PEAK
      WHERE ANCILLARY_SERVICE_ID = p_ANCILLARY_SERVICE_ID
...
BEGIN
   CACHE_ANCILLARY_SERVICE_ID;
   IF p_ANCILLARY_SERVICE_ID = l_PLC_ID THEN
      v_BEGIN_DATE := TO_DATE('6/1/' || p_PLAN_YEAR, c_DATE_FORMAT);
      v_END_DATE := ADD_MONTHS(TO_DATE('5/31/' || p_PLAN_YEAR, c_DATE_FORMAT),12);
   ELSIF p_ANCILLARY_SERVICE_ID = l_NSPL_ID THEN
      v_BEGIN_DATE := TO_DATE('1/1/'  || p_PLAN_YEAR, c_DATE_FORMAT);
      v_END_DATE :=  TO_DATE('12/31/' || p_PLAN_YEAR, c_DATE_FORMAT);
   END IF;
...
PROCEDURE CUT_PLC_NSPL_PEAK(p_ENTRY_ROWID IN VARCHAR2) AS
BEGIN
   DELETE ANCILLARY_SERVICE_AREA_PEAK WHERE ROWID = p_ENTRY_ROWID;
   COMMIT;
...
-- Process Each Line --
   FOR v_INDEX IN v_CONTAINER.FIRST..v_CONTAINER.LAST LOOP
      ...
      ASSERT(v_TOKENS.COUNT = 28, 'Invalid Record Format, Values Expected: 28, Observed: ' || v_TOKENS.COUNT);
      ...
      v_COUNT := v_COUNT + 1;
   END LOOP;
   p_MESSAGE := 'Number Of Records Posted To The CDI_PLC_ALMBACKS Table: ' || TO_CHAR(v_COUNT);
   LOGS.LOG_INFO(p_MESSAGE);
-- Stop The Process Log --
   LOGS.LOG_INFO(c_IMPORT_ALM_ADD_BACK_FILE || ' Complete. Elapsed Seconds: ' || TO_CHAR(ROUND((DBMS_UTILITY.GET_TIME-v_MARK_TIME)/100)));
   LOGS.STOP_PROCESS(p_MESSAGE, p_STATUS);
   COMMIT;
EXCEPTION
   WHEN OTHERS THEN
