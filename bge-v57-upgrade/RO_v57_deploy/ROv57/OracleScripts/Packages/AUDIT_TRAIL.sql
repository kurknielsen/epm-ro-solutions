...
-- Author  : PMANNING
-- Created : 4/17/2008 8:59:10 AM
-- Purpose : API for Auditing a new SYSTEM_TABLE
...
PROCEDURE REBUILD_ALL_TABLE_OBJECTS;
...
BEGIN
	...
	PUT('	v_PROC_ID := LOGS.CURRENT_AUDIT_PROCESS_ID;');
  
  --Check to see if Auditing is enabled
  PUT('	IF v_PROC_ID IS NOT NULL THEN');
	-- Handle Inserts
	...
	v_IDX := v_COLUMNS.FIRST;
	WHILE v_COLUMNS.EXISTS(v_IDX) LOOP
		PUT('			' || CASE WHEN v_IDX = v_COLUMNS.FIRST THEN 'IF ' ELSE ''
			END || 'NOT ((:NEW.' || v_COLUMNS(v_IDX) || ' IS NULL AND :OLD.' ||
			v_COLUMNS(v_IDX) || ' IS NULL) ' || 'OR :NEW.' || v_COLUMNS(v_IDX) ||
			' = :OLD.' || v_COLUMNS(v_IDX) || ')' || CASE WHEN
			v_IDX = v_COLUMNS.LAST THEN ' THEN' ELSE ' OR' END);
		v_IDX := v_COLUMNS.NEXT(v_IDX);
	END LOOP;
	PUT('			DECLARE');
	...
	BUILD_INSERT_STMT('OLD', 'F', '				', 'v_ID1', 'v_ID2');
	BUILD_INSERT_STMT('NEW', 'T', '				', 'v_ID2', 'v_ID1');
	PUT('			END;');
	PUT('		END IF;');
	-- Handle Deletes
	PUT('	ELSE');
...
-- Begin Building Dynamic Query
	...
	FOR v_SYSTEM_TABLE IN c_SYSTEM_TABLES LOOP
        ...
        v_SQL := v_SQL || ' AND A.ZAU_OPERATION != ''F''';
...
BEGIN
	...
	EXCEPTION
		WHEN NO_DATA_FOUND THEN
			ERRS.RAISE(MSGCODES.c_ERR_NO_SUCH_ENTRY, 'No Details found');
	END;
	...
	-- ZAU Operation
	v_SQL := v_SQL ||
			 'CASE A.ZAU_OPERATION WHEN ''I'' THEN ''Insert'' WHEN ''U'' THEN ''Update'' WHEN ''K'' THEN ''Insert [Key Update]'' WHEN ''X'' THEN ''Delete [Key Update]'' WHEN ''D'' THEN ''Delete'' WHEN ''T'' THEN ''Update To'' WHEN ''F'' THEN ''Update From'' ELSE ''Unknown'' END AS OPERATION, ';
	-- ZAU_TIMESTAMP
