CREATE OR REPLACE PACKAGE ML IS
--Revision $Revision: 1.9 $
--------------------------------------------------------------------------------------------------------------------------
-- MaiL Package
--------------------------------------------------------------------------------------------------------------------------
-- AUTHOR  : JHUMPHRIES
-- CREATED : 11/19/2004 11:22:25 AM
-- PURPOSE : This package provides higher-level methods that wrap the UTL_SMTP methods for sending e-mail
--          programmatically.
--
-- NOTES: This package is a slightly modified version of the demo_mail package provided by Oracle as an
--        example of how to use the UTL_SMTP package.
--        The original source can be downloaded from the following URL:
--          http://www.oracle.com/technology/sample_code/tech/pl_sql/htdocs/maildemo_sql.txt
--------------------------------------------------------------------------------------------------------------------------

-- record that defines email info for creating/sending e-mails
TYPE EMAIL_REC IS RECORD
	(
	EMAIL_ID           NUMBER,
	ATTACHMENT_COUNT   NUMBER,
	CURRENT_ATTACHMENT CLOB
	);

c_PRIORITY_HIGHEST 	CONSTANT PLS_INTEGER := 1;
c_PRIORITY_HIGH    	CONSTANT PLS_INTEGER := 2;
c_PRIORITY_NORMAL  	CONSTANT PLS_INTEGER := 3;
c_PRIORITY_LOW     	CONSTANT PLS_INTEGER := 4;
c_PRIORITY_LOWEST  	CONSTANT PLS_INTEGER := 5;

c_TO              	CONSTANT VARCHAR2(16) := 'To';
c_CC              	CONSTANT VARCHAR2(16) := 'Cc';
c_BCC              	CONSTANT VARCHAR2(16) := 'Bcc';

c_BASE64_ENCODING	CONSTANT VARCHAR2(16) := 'base64';

c_STATUS_PENDING    CONSTANT VARCHAR2(16) := 'Pending';
c_STATUS_QUEUED     CONSTANT VARCHAR2(16) := 'Queued';
c_STATUS_IN_PROCESS CONSTANT VARCHAR2(16) := 'In Process';
c_STATUS_SENT       CONSTANT VARCHAR2(16) := 'Sent';

--------------------------------------------------------------------------------------------------------------------------
-- Mail API
-----------
-- These routines create entries in the EMAIL_LOG table. When completed (via end_mail()),
-- the message will be sent or queued up for batch sending.
--------------------------------------------------------------------------------------------------------------------------
...
-- First, begin the email with begin_mail() and optionally call the add_*() methods
-- one or more times to define the recipients. Then call one of the attach_*() methods
-- to add the message. An alternate to using the attach_*() methods is to instead
-- call begin_attachment(), then call one of the write_*() methods one or more times to
-- create the attachment contents, and finally call end_attachment(). End the email
-- with end_mail().
FUNCTION BEGIN_MAIL
	(
	CATEGORY IN VARCHAR2,
...
-- with begin_attachment(). Then, call one of the write_*() methods
-- repeatedly to build the attachment piece-by-piece. End the attachment
-- with end_attachment().
...
-- Write email body in base64-encoded binary
...
-- End the attachment.
PROCEDURE END_ATTACHMENT(EMAIL IN OUT NOCOPY EMAIL_REC);
...
-- Processing
-------------
-- These routines allow for one or more entries in the EMAIL_LOG table to be sent.
-- The first will send all queued e-mails at once. The other sends a specific
-- e-mail, specified by ID.
--------------------------------------------------------------------------------------------------------------------------

-- This procedure will send appropriate Queued e-mails (based on date) in a batch fashion
PROCEDURE PROCESS_QUEUED;

-- This procedure will send or re-send a specific e-mail
PROCEDURE PROCESS_AN_EMAIL(EMAIL_ID IN NUMBER);
...
MAILER_ID VARCHAR2(256) := 'NewEnergy ML Mailer (Oracle UTL_SMTP)';
----------------------------------------------------------------------------------------------------
