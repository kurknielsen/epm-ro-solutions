...
-- Get the profiled usage over the consumption period.
	...
	SELECT (CASE WHEN v_BEGIN_DATE > p_PROFILE_BEGIN_DATE THEN v_BEGIN_DATE
			ELSE p_PROFILE_BEGIN_DATE + 1/NVL(p_PROFILE_INTERVAL,24)
			END)                     
    INTO v_BEGIN_DATE FROM DUAL; 
     
    LOGS.LOG_DEBUG('v_BEGIN_DATE=' ||  TEXT_UTIL.TO_CHAR_TIME(v_BEGIN_DATE));
    
    SELECT (CASE WHEN NVL(p_PROFILE_END_DATE,v_END_DATE) >= v_END_DATE THEN v_END_DATE
			ELSE NVL(p_PROFILE_END_DATE,v_END_DATE)+ 1
            END)  
    INTO v_END_DATE FROM DUAL;                    
    
    LOGS.LOG_DEBUG('v_END_DATE=' ||  TEXT_UTIL.TO_CHAR_TIME(v_END_DATE));
  ...
  LOGS.LOG_DEBUG('p_PROFILED_USAGE=' ||  p_PROFILED_USAGE);
	...
	IF LOGS.IS_DEBUG_DETAIL_ENABLED THEN
		LOGS.LOG_DEBUG('BILLED_USAGE=' || TO_CHAR(p_SERVICE_CONSUMPTION.BILLED_USAGE) || ', PROFILED_USAGE=' || TO_CHAR(p_PROFILED_USAGE) || ', FACTOR=' || TO_CHAR(v_FACTOR) || ', BEGIN_DATE=' || TO_CHAR(p_SERVICE_CONSUMPTION.BEGIN_DATE) || ', END_DATE=' || TO_CHAR(p_SERVICE_CONSUMPTION.END_DATE));
	END IF;
...
-- Allocate the consumption over the settlement period using the profiled usage.
	...
	FOR v_IDX IN 1..p_USAGE_DATES.COUNT LOOP
		IF p_USAGE_DATES(v_IDX) BETWEEN v_BEGIN_DATE AND v_END_DATE AND v_WITHIN_PERIOD_TBL(v_IDX) = 1 THEN
			v_INTERVAL := ROUND((p_USAGE_DATES(v_IDX) - v_BASE_DATE) / v_PROFILE_INTERVAL_DIVISOR) + 1;
