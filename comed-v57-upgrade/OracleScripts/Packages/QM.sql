CREATE OR REPLACE PACKAGE QM AS
...
-- BUG 12759 - 2/13/2007 - pbm - Added these 2 procs from the
-- old IT package. They were still being called in QuoteManagement VB.
...
g_TD_LOSS_FACTOR NUMBER := 1.0;
g_NUMBER_OF_CUSTOMERS NUMBER(8) := 1;
g_LARGE_STRING NUMBER(8) := 512;
g_SECOND NUMBER(6,5) :=  .00001;

TYPE STRING_TABLE_LARGE IS TABLE OF VARCHAR(512) INDEX BY BINARY_INTEGER;

TYPE SEGMENT_RECORD IS RECORD
    (
	SEGMENT_QUANTITY NUMBER(8,2),
	SEGMENT_COST NUMBER(10,2),
	SEGMENT_ORDER NUMBER(4),
	SEGMENT_COUNT NUMBER(6)
	);

TYPE SEGMENT_TABLE IS TABLE OF SEGMENT_RECORD INDEX BY BINARY_INTEGER;

TYPE DAY_TYPE_RECORD IS RECORD
    (
	WD_DAYS NUMBER(2),
	WE_DAYS NUMBER(2)
	);

TYPE DAY_TYPE_TABLE IS TABLE OF DAY_TYPE_RECORD INDEX BY BINARY_INTEGER;
END;
/
