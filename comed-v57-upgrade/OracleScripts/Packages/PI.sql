CREATE OR REPLACE PACKAGE PI AS
--Revision $Revision: 1.72 $

-- Profiling Interface Package --.
...
g_STRING_ALL VARCHAR2(8) := '<ALL>';
g_UNKNOWN NUMBER(4) := -999;
g_DOMAIN_NAME VARCHAR2(16) := 'Profiling';
g_APP_NAME VARCHAR2(16) := 'RetailOffice';
g_SECOND NUMBER(6,5) := 1/86400;
g_PRIMARY_PARAMETER_CATEGORY  VARCHAR2(16):= 'Temperature';
...
BEGIN
	...
	OPEN p_CURSOR FOR
	    SELECT A.ACCOUNT_ID,
			  A.ACCOUNT_NAME || DECODE(A.IS_EXTERNAL_INTERVAL_USAGE,1,' External',' Internal') "NAME_INTERNAL_EXTERNAL"
...
PROCEDURE PUT_LP_RESULTANT_STATISTICS
	...
	CURSOR c_RESULTANT_STATS IS
		SELECT COUNT(POINT_VAL) "RCOUNT",
			(AVG(POINT_VAL) / MAX(POINT_VAL)) "RLOADFACTOR",
			MIN(POINT_VAL) "RMIN",
