CREATE OR REPLACE PACKAGE ITJ AS
...
-- Retrieve Schedule Management mapping data.
-- %param p_TRANSACTION_IDs	IDs of Transaction for whom to show mappings.
--					Can include CONSTANTS.ALL_ID.
-- %param p_STATEMENT_TYPE_ID	ID of Statement Type for whom to show
--					mappings. Can be CONSTANTS.ALL_ID.
-- %param p_SCHEDULE_STATE		Schedule state for whom to show
--					mappings. Can be CONSTANTS.ALL_ID.
-- %param p_SCHED_MGMT_CIDs	IDs of Schedules for whom to show mappings.
--					Can include CONSTANTS.ALL_STRING.
-- %param p_SCHED_MGMT_DATA_SOURCE Data Source for whom to show
--					mappings. Can be CONSTANTS.ALL_STRING.
-- %param p_CURSOR			The queried results.
...
-- Push schedule data to Schedule Management to synchronize.
-- %param p_BEGIN_DATE		First day of the date range to push.
-- %param p_END_DATE		Last day of the date range to push.
-- %param p_TIME_ZONE		Time zone of the date range.
-- %param p_TRANSACTION_IDs	IDs of Transactions to push.
-- %param p_STATEMENT_TYPE_IDs	IDs of Statement Types to push.
-- %param p_SCHEDULE_STATEs	Schedule states to push.
--	The above parameters must be parallel collections ¿ all with the same
--	number of elements. A given value from each collection at the same index
--	form a record that identifies a schedule to push.
-- %param p_MESSAGE			A result message.
...
-- Returns the list of schedule IDs already mapped.
-- %param p_SEARCH_STRING	Search string for filtering list of IDs
...
-- Returns the number of distinct schedule IDs already mapped.
FUNCTION GET_SCHED_MGMT_CID_COUNT RETURN NUMBER;

-- Determine the POD_ID for the specified transaction. Returns NULL if
-- the specified transaction ID is not valid.
-- %param p_TRANSACTION_ID	The transaction in question
-- %return				The POD_ID field of the specified
--					transaction
FUNCTION GET_POD_ID(p_TRANSACTION_ID IN NUMBER) RETURN NUMBER;

g_LINEFEED CHAR(1) := CHR(10);
g_MODULE_NAME VARCHAR(16) := 'SCHEDULING';
g_REDIRECT_SCHEDULER BOOLEAN := FALSE;

c_LEVEL_SUBSTATION				CONSTANT NUMBER(1) := 1;
c_LEVEL_SUBSTATION_METER		CONSTANT NUMBER(1) := 2;
c_LEVEL_SUBSTATION_METER_POINT	CONSTANT NUMBER(1) := 3;

c_ANYDAY_STRING VARCHAR2(7) := '1111111';
c_WEEKEND_STRING VARCHAR2(7) := '0000011';
c_WEEKDAY_STRING VARCHAR2(7) := '1111100';

c_LOCAL_ON_PEAK_TEMPLATE	CONSTANT VARCHAR2(32) := 'Local On-Peak';
c_LOCAL_OFF_PEAK_TEMPLATE	CONSTANT VARCHAR2(32) := 'Local Off-Peak';
