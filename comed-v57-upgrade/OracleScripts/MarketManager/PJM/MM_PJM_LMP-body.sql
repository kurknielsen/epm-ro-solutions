...
BEGIN
    ...
    WHILE p_RECORDS.EXISTS(v_IDX) LOOP
        IF v_LAST_PNODE_ID != p_RECORDS(v_IDX).PNODEID THEN
            v_LAST_PNODE_ID := p_RECORDS(v_IDX).PNODEID;
            v_MP_ID_LMP := GET_MARKET_PRICE_ID(p_RECORDS(v_IDX).PNODEID, v_MARKET_TYPE, g_LMP);
            v_MP_ID_MCC := GET_MARKET_PRICE_ID(p_RECORDS(v_IDX).PNODEID, v_MARKET_TYPE, g_CONG_COMPONENT);
            v_MP_ID_MLC := GET_MARKET_PRICE_ID(p_RECORDS(v_IDX).PNODEID, v_MARKET_TYPE, g_LOSS_COMPONENT);
        END IF;
        ...
        IF v_MP_ID_LMP IS NOT NULL THEN
            ...
            WHILE v_TOTAL_PRICES.EXISTS(v_TTL_PRICE_IDX) LOOP
                ...
                FOR I IN v_PRICE_CODE_ARRAY.FIRST .. v_PRICE_CODE_ARRAY.LAST LOOP
                    ...
                    --Update DayAhead or RealTime Generation Schedule with price
                    MM_PJM_UTIL.ADD_PRICE_TO_GEN_SCHED(v_MPV_ROW.PRICE_DATE, v_MARKET_TYPE, p_RECORDS(v_IDX).PNODEID, v_MPV_ROW.PRICE, p_STATUS, p_MESSAGE);       
                END LOOP;
...
BEGIN
    ...
    IF p_IMPORT_TYPE IN ('DA','RT') THEN
        MEX_PJM_LMP.PARSE_LMP_ML(p_DATE, p_IMPORT_FILE, 0, v_ENG_RECORDS, v_RECORDS, p_STATUS, p_MESSAGE);

        IF p_STATUS = GA.SUCCESS THEN
            IMPORT_LMP_ML(p_IMPORT_TYPE, v_ENG_RECORDS, v_RECORDS, 0, p_STATUS, p_MESSAGE);
        END IF;
		...
		LOOP
			CASE v_ACTION
				...
				WHEN g_ET_DAY_AHEAD_LMP_MONTH THEN
					v_MARKET_TYPE := 'DA';
					QUERY_LMP(v_MARKET_TYPE, v_CURRENT_DATE, 1, p_STATUS, p_MESSAGE, v_LOGGER);
...
BEGIN
	...
	CASE p_EXCHANGE_TYPE
		WHEN g_ET_DA_LMP_FROM_FILE THEN
			IMPORT_LMP_ML_FROM_FILE(p_BEGIN_DATE, p_IMPORT_FILE, 'DA', p_STATUS, p_MESSAGE);
        WHEN g_ET_RT_LMP_FROM_FILE THEN
			IMPORT_LMP_ML_FROM_FILE(p_BEGIN_DATE, p_IMPORT_FILE, 'RT', p_STATUS, p_MESSAGE);
        WHEN g_ET_FTR_LMP_FROM_FILE THEN
			IMPORT_LMP_ML_FROM_FILE(p_BEGIN_DATE, p_IMPORT_FILE, 'FTR', p_STATUS, p_MESSAGE);
		ELSE
			p_STATUS := -1;
			p_MESSAGE := 'Exchange Type ' || p_EXCHANGE_TYPE || ' not found.';
			v_LOGGER.LOG_ERROR(p_MESSAGE);
