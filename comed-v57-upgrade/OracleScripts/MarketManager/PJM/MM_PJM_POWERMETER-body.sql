...
----------------------------------------------------------------------------------------------------
PROCEDURE IMPORT_PECO_FINAL_ZONE
    (
   p_XML           IN XMLTYPE,
   p_PROCESS_STATUS     OUT NUMBER,
   p_MESSAGE            OUT VARCHAR2
...
BEGIN
    LOGS.START_PROCESS('Import PJM Power Meter',
        ...
        WHEN 'LoadSubmissionDetails' THEN
            CASE p_EXCHANGE_TYPE
                WHEN 'Import Power Meter File' THEN
                    IMPORT_LOAD_SUBMISSION_DTL(v_XML, p_PROCESS_STATUS, p_MESSAGE);
                WHEN 'Query Submissions' THEN
                    IMPORT_LOAD_SUBMISSION_DTL(v_XML, p_PROCESS_STATUS, p_MESSAGE);                    
                WHEN 'Query EOM Load With Losses' THEN
                    IMPORT_EOM_LOAD_WITH_LOSSES(v_XML, p_PROCESS_STATUS, p_MESSAGE);
                WHEN 'Query PECO Final Zone Load and Interchange' THEN
                    IMPORT_PECO_FINAL_ZONE(v_XML, p_PROCESS_STATUS, p_MESSAGE);
                WHEN 'Query Total Generation' THEN
                    IMPORT_TOTAL_GENERATION(v_XML, p_PROCESS_STATUS, p_MESSAGE);
                END CASE;
        WHEN 'MeterValues' THEN
            IMPORT_XML(v_XML, p_PROCESS_STATUS, p_MESSAGE);
        WHEN 'MeterValueAllocation' THEN
            CASE p_EXCHANGE_TYPE
                WHEN 'Import Power Meter File' THEN
                    IMPORT_ALLOCATED_METER_VALUES(v_XML, p_PROCESS_STATUS, p_MESSAGE);
                WHEN 'Query Allocations' THEN
                    IMPORT_ALLOCATED_METER_VALUES(v_XML, p_PROCESS_STATUS, p_MESSAGE);                    
                WHEN 'Query 500kV Losses' THEN
                    IMPORT_500KV_LOSSES(v_XML, p_PROCESS_STATUS, p_MESSAGE);
                END CASE;
        ELSE p_MESSAGE := p_MESSAGE ||'Unable to recognize file '||v_FILE_NAME || ' as Power Meter XML.';
...
BEGIN
    ...
    --Are we connecting to the sandbox?
