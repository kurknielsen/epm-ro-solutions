		...
		OPEN p_CURSOR FOR
			...
			FROM
				...
				WHERE (p_ESP_ID = g_NA_BRANCH OR A.ESP_ID = p_ESP_ID)
                    ...
                    --rss 2015/08/20
                    and acct.ACCOUNT_MODEL_OPTION <> 'Aggregate'
				UNION
...
--@@Augment The Thumbnail With Loss Factor Values --
         DECLARE
         ...
         BEGIN
            SELECT MAX(LOSS_FACTOR_ID) INTO v_LOSS_FACTOR_ID FROM ACCOUNT_LOSS_FACTOR WHERE CASE_ID = GA.BASE_CASE_ID AND ACCOUNT_ID = p_ACCOUNT_ID AND TRUNC(p_BEGIN_DATE) BETWEEN BEGIN_DATE AND NVL(END_DATE, CONSTANTS.HIGH_DATE);
            IF v_LOSS_FACTOR_ID IS NOT NULL THEN
               SELECT LOSS_FACTOR_NAME INTO v_LOSS_FACTOR_NAME FROM LOSS_FACTOR WHERE LOSS_FACTOR_ID = v_LOSS_FACTOR_ID;
               v_DX_LOSS_FACTOR := CALC_UTIL.GET_LOSS_FACTOR(v_LOSS_FACTOR_ID, TRUNC(p_BEGIN_DATE), p_LOSS_TYPE => CALC_UTIL.c_LOSS_TYPE_DISTRIBUTION, p_FACTOR_TYPE => CALC_UTIL.c_FACTOR_TYPE_EXPANSION);
               v_TX_LOSS_FACTOR := CALC_UTIL.GET_LOSS_FACTOR(v_LOSS_FACTOR_ID, TRUNC(p_BEGIN_DATE), p_LOSS_TYPE => CALC_UTIL.c_LOSS_TYPE_TRANSMISSION, p_FACTOR_TYPE => CALC_UTIL.c_FACTOR_TYPE_EXPANSION);
               OPEN p_CURSOR FOR
                  ...
                  UNION SELECT 'Loss Factor' "Attribute", TO_CHAR(v_LOSS_FACTOR_NAME) "Value" FROM DUAL
                  UNION SELECT 'Loss Factor DX' "Attribute", TO_CHAR(v_DX_LOSS_FACTOR) "Value" FROM DUAL
                  UNION SELECT 'Loss Factor TX' "Attribute", TO_CHAR(v_TX_LOSS_FACTOR) "Value" FROM DUAL;
               RETURN;
            END IF;
         END;
