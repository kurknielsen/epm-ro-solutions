...
TYPE t_ANYTIME_PERIOD_TEMPLATE IS TABLE OF BOOLEAN INDEX BY PLS_INTEGER;
l_ANYTIME_PERIOD_TEMPLATE t_ANYTIME_PERIOD_TEMPLATE;
--l_LOG_PROXY_DAY_ASSIGN_STATS BOOLEAN := NVL(UPPER(SUBSTR(GET_DICTIONARY_VALUE('Log Proxy Day Assignment Statistics', CDI_CONSTANTS.c_SYSTEM_SETTING_LIBRARY, 'Custom Data Interface', 'Override', 'Forecast/Backcast'),1,1)), CDI_CONSTANTS.c_NO) = CDI_CONSTANTS.c_YES;
...
--@@Begin Implementation Override --
--@@Add Forall Block Exception Handling --
   DECLARE
   v_ERROR_INDEX BINARY_INTEGER;
   v_HAVE_ERRORS BOOLEAN := FALSE;
   BEGIN
--@@Remove Any Content That Will Be Replaced By The Insert --
	FORALL v_INDEX IN 1..g_SERVICE_LOAD_ID.COUNT
         DELETE SERVICE_LOAD
         WHERE SERVICE_ID = g_SERVICE_LOAD_ID(v_INDEX)
            AND SERVICE_CODE = p_CAST_CONTEXT.SERVICE_CODE
            AND LOAD_DATE = g_SERVICE_LOAD_DATE(v_INDEX)
            AND LOAD_CODE = v_LOAD_CODE;
      FORALL v_INDEX IN 1..g_SERVICE_LOAD_ID.COUNT SAVE EXCEPTIONS
   ...
   EXCEPTION
      WHEN OTHERS THEN
         FOR i IN 1..SQL%BULK_EXCEPTIONS.COUNT LOOP
            ...
            LOGS.LOG_ERROR('Service Id: ' || TO_CHAR(g_SERVICE_LOAD_ID(v_ERROR_INDEX)) || ', Load Date: ' || TEXT_UTIL.TO_CHAR_TIME(g_SERVICE_LOAD_DATE(v_ERROR_INDEX)) || ', Message: ' || SQLERRM(-SQL%BULK_EXCEPTIONS(i).ERROR_CODE));
            v_HAVE_ERRORS := TRUE;
         END LOOP;
      IF v_HAVE_ERRORS THEN
         ERRS.RAISE_BUSINESS_RULE_ERROR(' Processing Terminated Due To Service Load Store Exception. See Process Log For Details.');
      END IF;
   END;
--@@End Implementation Override --
...
-- v_DELETE_EXTERNAL NUMBER(1);
-- v_NOT_LOAD_CODE CHAR(1);
--
-- BEGIN
--
-- 	--GET DATE RANGE FOR SERVICE_LOAD DELETE
-- 	UT.CUT_DATE_RANGE(GA.DEFAULT_MODEL, p_CAST_CONTEXT.BEGIN_DATE, p_CAST_CONTEXT.END_DATE, LOCAL_TIME_ZONE, v_HOURLY_BEGIN_DATE, v_HOURLY_END_DATE);
--
-- 	--DETERMINE WHETHER WE DELETE EXTERNAL SERVICE_LOAD
--     SELECT DECODE(VALUE,0,GA.EXTERNAL,'%')
--     INTO v_NOT_LOAD_CODE
--     FROM SYSTEM_DICTIONARY
--     WHERE MODEL_ID = 0
--         AND KEY1 = 'Forecasting'
--         AND KEY2 = 'DELETE_EXTERNAL_FORECAST';
--
-- -- Delete for Single Account Run.
-- 	IF p_CAST_CONTEXT.ACCOUNT_PROCESS_MODE = 0 THEN
-- 		SELECT ID_TYPE(A.SERVICE_ID)
-- 		BULK COLLECT INTO v_IDS
-- 		FROM SERVICE A, SERVICE_STATE B
-- 		WHERE A.MODEL_ID = p_CAST_CONTEXT.MODEL_ID
-- 			AND A.SCENARIO_ID = p_CAST_CONTEXT.SCENARIO_ID
-- 			AND A.AS_OF_DATE = p_CAST_CONTEXT.OUTPUT_AS_OF_DATE
-- 			AND A.ACCOUNT_SERVICE_ID IN (SELECT ACCOUNT_SERVICE_ID FROM ACCOUNT_SERVICE WHERE ACCOUNT_ID = p_CAST_CONTEXT.ACCOUNT_ID)
-- 			AND B.SERVICE_ID = A.SERVICE_ID
--  			AND B.SERVICE_CODE = p_CAST_CONTEXT.REQUEST_TYPE
--  			AND B.SERVICE_DATE BETWEEN p_CAST_CONTEXT.BEGIN_DATE AND p_CAST_CONTEXT.END_DATE;
-- 	ELSIF p_CAST_CONTEXT.EDC_ID = g_ALL AND p_CAST_CONTEXT.ESP_ID = g_ALL THEN
-- -- Delete for the Type of Run: 1-All, 2-Agg, 3-Non-Agg, 4-External.
-- 		SELECT ID_TYPE(X.SERVICE_ID)
-- 		BULK COLLECT INTO v_IDS
-- 		FROM (SELECT DISTINCT A.SERVICE_ID
-- 			FROM SERVICE A, SERVICE_STATE B
-- 			WHERE A.MODEL_ID = p_CAST_CONTEXT.MODEL_ID
-- 				AND A.SCENARIO_ID = p_CAST_CONTEXT.SCENARIO_ID
-- 				AND A.AS_OF_DATE = p_CAST_CONTEXT.OUTPUT_AS_OF_DATE
...
-- 	 			AND B.SERVICE_CODE = p_CAST_CONTEXT.REQUEST_TYPE
-- 	 			AND B.SERVICE_DATE BETWEEN p_CAST_CONTEXT.BEGIN_DATE AND p_CAST_CONTEXT.END_DATE
-- 	 			AND B.IS_AGGREGATE_ACCOUNT = DECODE(p_CAST_CONTEXT.ACCOUNT_PROCESS_MODE, 1, IS_AGGREGATE_ACCOUNT, 2, 1, 3, 0, 4, IS_AGGREGATE_ACCOUNT, 9)) X;
-- 	ELSE
-- 		SELECT ID_TYPE(A.SERVICE_ID)
-- 		BULK COLLECT INTO v_IDS
-- 		FROM SERVICE A, SERVICE_STATE B
...
-- 	END IF;
--
-- 	DELETE /*+ ORDERED INDEX(A) USE_NL(X,A) */ SERVICE_LOAD A
-- 	WHERE SERVICE_ID IN (SELECT X.ID FROM TABLE(CAST(v_IDS AS ID_TABLE)) X)
-- 		AND SERVICE_CODE = p_CAST_CONTEXT.REQUEST_TYPE
-- 		AND LOAD_DATE BETWEEN v_HOURLY_BEGIN_DATE AND v_HOURLY_END_DATE
-- 		AND NOT LOAD_CODE = v_NOT_LOAD_CODE;
--
-- 	DELETE /*+ ORDERED INDEX(A) USE_NL(X,A) */ SERVICE_STATE A
-- 	WHERE SERVICE_ID IN (SELECT X.ID FROM TABLE(CAST(v_IDS AS ID_TABLE)) X)
-- 			AND SERVICE_CODE = p_CAST_CONTEXT.REQUEST_TYPE
-- 			AND SERVICE_DATE BETWEEN p_CAST_CONTEXT.BEGIN_DATE AND p_CAST_CONTEXT.END_DATE;
--
-- 	IF LOGS.IS_DEBUG_ENABLED THEN
...
-- Delete for Single Account Run.
	...
	--@@ rss 	UT.CUT_DATE_RANGE(p_CAST_CONTEXT.MODEL_ID, p_CAST_CONTEXT.BEGIN_DATE, p_CAST_CONTEXT.END_DATE, GA.LOCAL_TIME_ZONE, v_CUT_BEGIN, v_CUT_END);
   ...
   -- @@ rss 2012-11-12
    ELSIF p_CAST_CONTEXT.ACCOUNT_GROUP_ID > 0 THEN
      delete SERVICE_LOAD
        WHERE ROWID IN
            (SELECT SL.ROWID
             FROM SERVICE S, SERVICE_LOAD SL
             WHERE S.ACCOUNT_SERVICE_ID IN
                (SELECT ACCOUNT_SERVICE_ID
                   FROM ACCOUNT_SERVICE A, ACCOUNT_GROUP_ASSIGNMENT B
                   WHERE B.ACCOUNT_GROUP_ID = p_CAST_CONTEXT.ACCOUNT_GROUP_ID
		...
		WHERE ROWID IN
			(SELECT SS.ROWID
			 FROM SERVICE S, SERVICE_STATE SS
			 WHERE S.ACCOUNT_SERVICE_ID IN
                (SELECT ACCOUNT_SERVICE_ID
                   FROM ACCOUNT_SERVICE A, ACCOUNT_GROUP_ASSIGNMENT B
                   WHERE B.ACCOUNT_GROUP_ID = p_CAST_CONTEXT.ACCOUNT_GROUP_ID
...
--@@Begin Implementation Override:  Carry over from 3.3 override
--  Comment out IF condition and always set ACCUMULATE_SOB_LOAD := 0;
/*
...
*/
      p_CAST_CONTEXT.ACCUMULATE_SOB_LOAD := 0;
--@@End Implementation Override:  Carry over from 3.3 override
	...
	IF p_CAST_CONTEXT.REQUEST_MODE = GA.ACCOUNT_GROUP_MODE THEN
      ...
      --@@Begin Implementation Override:  Carry over from 3.3 override
		p_CAST_CONTEXT.ACCOUNT_PROCESS_MODE := 1; --ALL
		p_CAST_CONTEXT.GENERATE_SUMMARY := 1;
      --@@End Implementation Override
	END IF;
...
--@@Begin Implementation Override --
--@@Answer True If The Template Is An Anytime Period Template; Otherwise Answer False --
   FUNCTION IS_ANYTIME_PERIOD_TEMPLATE RETURN BOOLEAN AS
   v_COUNT PLS_INTEGER := 0;
   v_PERIOD_ID NUMBER(9);
   v_TEMPLATE_NAME TEMPLATE.TEMPLATE_NAME%TYPE;
   CURSOR c_SELECT IS
      SELECT PERIOD_ID, COUNT(*)
      FROM TEMPLATE_DATES TD, TEMPLATE_DAY_TYPE_PERIOD DT
      ...
      GROUP BY PERIOD_ID;
   BEGIN
      IF NOT l_ANYTIME_PERIOD_TEMPLATE.EXISTS(p_SERVICE_CONSUMPTION.TEMPLATE_ID) THEN
         SELECT TEMPLATE_NAME INTO v_TEMPLATE_NAME FROM TEMPLATE WHERE TEMPLATE_ID = p_SERVICE_CONSUMPTION.TEMPLATE_ID;
         FOR v_SELECT IN c_SELECT LOOP
            v_COUNT := v_COUNT + 1;
            v_PERIOD_ID := v_SELECT.PERIOD_ID;
         END LOOP;
         l_ANYTIME_PERIOD_TEMPLATE(p_SERVICE_CONSUMPTION.TEMPLATE_ID) := v_COUNT = 1 AND v_PERIOD_ID = 1;
         LOGS.LOG_INFO('Template "' || v_TEMPLATE_NAME || CASE WHEN l_ANYTIME_PERIOD_TEMPLATE(p_SERVICE_CONSUMPTION.TEMPLATE_ID) THEN '" Is ' ELSE '" Is Not ' END || 'An Anytime Period Template', 'BILLED_USAGE_ALLOCATION', 'IS_ANYTIME_PERIOD_TEMPLATE', 'FS');
      END IF;
      RETURN l_ANYTIME_PERIOD_TEMPLATE(p_SERVICE_CONSUMPTION.TEMPLATE_ID);
   END IS_ANYTIME_PERIOD_TEMPLATE;
--@@End Implementation Override --
BEGIN
...
--@@Begin Implementation Override --
--@@The Query Used To Assign TOU Period To Each Interval Is A Performance Bottleneck And Unnecessry When The Template Is An Anytime Period Template --
   IF IS_ANYTIME_PERIOD_TEMPLATE THEN
      SELECT USAGE_DATE, USAGE_VAL, 1
      BULK COLLECT INTO v_USAGE_DATES, v_USAGE_VALS, v_WITHIN_PERIOD_TBL
      FROM TABLE(CAST(p_PROFILE AS USAGE_TABLE));
   ELSE
...
--@@End Implementation Override --
   ...
   IF LOGS.IS_DEBUG_ENABLED THEN --Implementation Override --
      LOGS.LOG_DEBUG('BILLED_USAGE=' || TO_CHAR(p_SERVICE_CONSUMPTION.BILLED_USAGE) || ', PROFILED_USAGE=' || TO_CHAR(v_PROFILED_USAGE) || ', BEGIN_DATE=' || TO_CHAR(p_SERVICE_CONSUMPTION.BEGIN_DATE) || ', END_DATE=' || TO_CHAR(p_SERVICE_CONSUMPTION.END_DATE)  || ', FACTOR=' || TO_CHAR(v_FACTOR));
	END IF;
...
-- Cull out the allocation for the current service date in preparation for applying losses.
	...
	END IF;
 	...
 	IF LOGS.IS_DEBUG_DETAIL_ENABLED THEN
		LOGS.LOG_DEBUG_DETAIL('ASSIGN_USAGE_ALLOCATION TIME=' || TO_CHAR(DBMS_UTILITY.GET_TIME - v_ELAPSED) || ', SERVICE DATE=' || TO_CHAR(p_SERVICE_DATE) || ', COUNT=' || TO_CHAR(v_LOAD_DATE.COUNT) || ', LOAD=' || TO_CHAR(v_LOAD));
	END IF;
...
BEGIN
	...
	WHERE EXISTS
 	...
 	IF LOGS.IS_DEBUG_ENABLED THEN
		LOGS.LOG_DEBUG('DELETE_ACTUAL_SERVICE TIME=' || TO_CHAR(DBMS_UTILITY.GET_TIME - v_ELAPSED) || ', ACCOUNT_SERVICE_ID=' || TO_CHAR(p_ACCOUNT_SERVICE_ID) || ', PROVIDER_SERVICE_ID=' || TO_CHAR(p_PROVIDER_SERVICE_ID));
	END IF;
		...
		BEGIN
			IF v_ACCOUNT_SERVICE_ID > 0 AND v_PROFILE_ID > 0 THEN
				...
				FOR v_CONSUMPTION_SEGMENT IN c_CONSUMPTION_SEGMENT LOOP
				     BILLED_USAGE_ALLOCATION(p_CAST_CONTEXT, v_CONSUMPTION_SEGMENT.CS_TYPE, v_PROFILE, v_PROFILE_INTERVAL, v_USAGE_ALLOCATION, v_USAGE_ALLOCATION_XREF);
				END LOOP;
...
-- Option to only Rollup the cast detail to summary level.
	...
	FROM (SELECT DISTINCT D.PROFILE_ID, F.ACCOUNT_SERVICE_ID, F.PROVIDER_SERVICE_ID, F.SERVICE_DELIVERY_ID, H.BEGIN_DATE, H.END_DATE, H.CONSUMPTION_ID
		...
		WHERE (p_CAST_CONTEXT.ACCOUNT_ID = g_ALL OR A.ACCOUNT_ID = p_CAST_CONTEXT.ACCOUNT_ID)
			...
			AND H.BILL_CODE = GA.BILL_CONSUMPTION
			AND H.CONSUMPTION_CODE = GA.ACTUAL_CONSUMPTION
			AND H.UNIT_OF_MEASUREMENT = GA.DEFAULT_UNIT_OF_MEASUREMENT
		UNION ALL SELECT DISTINCT G.PROFILE_ID, I.ACCOUNT_SERVICE_ID, I.PROVIDER_SERVICE_ID, I.SERVICE_DELIVERY_ID, K.BEGIN_DATE, K.END_DATE, K.CONSUMPTION_ID
		FROM ACCOUNT A, ACCOUNT_STATUS B, ACCOUNT_STATUS_NAME STATUS_NAME, ACCOUNT_SERVICE_LOCATION C, SERVICE_LOCATION_METER D, METER E, ACCOUNT_STATUS_NAME MSN, METER_CALENDAR F, CALENDAR_PROFILE G, ACCOUNT_SERVICE H, SERVICE I, PROVIDER_SERVICE J, SERVICE_CONSUMPTION K
		WHERE (p_CAST_CONTEXT.ACCOUNT_ID = g_ALL OR A.ACCOUNT_ID = p_CAST_CONTEXT.ACCOUNT_ID)
			AND UPPER(SUBSTR(A.ACCOUNT_MODEL_OPTION,1,1)) = 'M'
			...
			AND K.BILL_CODE = GA.BILL_CONSUMPTION
			AND K.CONSUMPTION_CODE = GA.ACTUAL_CONSUMPTION
			AND K.UNIT_OF_MEASUREMENT = GA.DEFAULT_UNIT_OF_MEASUREMENT) A;
...
BEGIN
	...
	IF v_CAST_CONTEXT.REQUEST_TYPE = 'A' THEN
		CONSUMPTION_ALLOCATION_REQUEST(v_CAST_CONTEXT);
...
--@@Begin Implementation Override -- KN 10/5/2012 QC1505 (From 3.3 override code)
--	SELECT DECODE(v_CAST_CONTEXT.REQUEST_TYPE, GA.FORECAST_SERVICE, GA.FORECAST_SERVICE, GA.ACTUAL_SERVICE) INTO v_CAST_CONTEXT.WEATHER_PARAMETER_CODE FROM DUAL;
 	v_CAST_CONTEXT.WEATHER_PARAMETER_CODE := 'A';
--@@End Implementation Override --
