...
BEGIN
 ...
 --@@Implementation Override Begin - This stmt causes a 'No Rows Found' Error and seems to server no purpuse
   -- p_MESSAGE := TO_CHAR(MAX_DATA_LENGTH('APP_EVENT_LOG','EVENT_OBJECT_NAME'));
 --@@Implementation Override End
...
BEGIN
	...
	BEGIN
      --@@Implementation Override-- KN 9/5/2012 add NVL function
      SELECT nvl(IS_EXTERNAL_INTERVAL_USAGE,0) INTO v_IS_EXTERNAL_INTERVAL_USAGE
...
BEGIN
	...
	FOR v_IDX IN 1..p_CANDIDATE.NUM_SEGMENTS LOOP
		...
		ELSE
         ...
         ----RSS 2017/03/12
         IF v_SEGMENT_LIMITS(v_IDX-1).Y > v_SEGMENT_LIMITS(v_IDX).Y THEN
           IF LOGS.IS_DEBUG_ENABLED THEN
              LOGS.LOG_INFO('FCM_CALCULATE_CLUSTERS SEGMENT_MAX < SEGMENT_MIN:'||v_SEGMENT_LIMITS(v_IDX-1).Y ||'>'||v_SEGMENT_LIMITS(v_IDX).Y);
              LOGS.LOG_INFO('FCM_CALCULATE_CLUSTERS p_CANDIDATE.NUM_SEGMENTS: '||p_CANDIDATE.NUM_SEGMENTS||' p_CANDIDATE.FCM_ID='||p_CANDIDATE.FCM_ID);
              LOGS.LOG_INFO('CLUSTER_NUM: '||(v_IDX-1));
           END IF;
           UPDATE WRF_FCM_CLUSTER SET SEGMENT_MAX = v_MAX
             WHERE FCM_ID = p_CANDIDATE.FCM_ID
            AND NUM_SEGMENTS = p_CANDIDATE.NUM_SEGMENTS
            AND CLUSTER_NUM = v_IDX-1;
         END IF;
		...
		IF v_IDX = v_CLUSTERS.LAST THEN
          ...
          ----RSS 2017/03/12
         IF v_SEGMENT_LIMITS(v_IDX).Y > v_MAX THEN
           IF LOGS.IS_DEBUG_ENABLED THEN
              LOGS.LOG_INFO('last cluster v_SEGMENT_LIMITS(v_IDX).Y > v_MAX:'||v_SEGMENT_LIMITS(v_IDX).Y ||'>'||v_MAX);
              LOGS.LOG_INFO('FCM_CALCULATE_CLUSTERS p_CANDIDATE.NUM_SEGMENTS: '||p_CANDIDATE.NUM_SEGMENTS||' p_CANDIDATE.FCM_ID='||p_CANDIDATE.FCM_ID);
           END IF;
         END IF;
		END IF;
...
CURSOR cur_OBSERVATIONS IS
	...
	FROM WRF_FCM_CLUSTER_ASSIGNMENT A,
		WRF_FCM_OBSERVATION O
	WHERE A.FCM_ID = p_FCM_ID
		AND A.NUM_SEGMENTS = p_NUM_SEGMENTS
		AND A.CLUSTER_NUM = p_CLUSTER_NUM
		AND O.FCM_ID = p_FCM_ID
		AND O.OBSERVATION_NUM = A.OBSERVATION_NUM
		...
		FOR v_CLUSTER IN 1..v_NUM_SEGMENTS LOOP
			...
			IF v_COEFF_ALL_ZEROES = TRUE THEN
				...
				LOGS.LOG_WARN(p_EVENT_TEXT => 'Too few observations(' || v_Y.COUNT
							...
							|| ', Segment = #' || v_CLUSTER
							|| ' at Hour = ' || p_HOUR
              || ' Create_profile: v_CANDIDATE.FCM_ID='||v_CANDIDATE.FCM_ID
...
BEGIN
    ...
    -- get existing or create new library and profile
   --@@Implementation Override Begin:  KN 9/7/2012  Append Sysdate: carried over from 3.3 override.
   PUT_LOAD_PROFILE_LIBRARY(p_PREFIX_NAME || '_Library_'|| TO_CHAR(SYSDATE,'YYYY_MM_DD'), v_PROFILE_LIBRARY_ID);
   v_PROFILE_NAME := p_PREFIX_NAME || '_' || v_TEMPLATE_NAME|| '_'|| TO_CHAR(SYSDATE,'YYYY_MM_DD');
   --@@Implementation Override End:  KN 9/7/2012  Append Sysdate: carried over from 3.3 override.
...
v_TEMPLATE_NAME VARCHAR2(64); --@@Implementation Override--
...
--@@Begin Implementation Override --
    IF v_BREAKPOINT_ID IS NULL THEN
        SELECT MAX(TEMPLATE_NAME) INTO v_TEMPLATE_NAME FROM TEMPLATE WHERE TEMPLATE_ID = p_TEMPLATE_ID;
        LOGS.LOG_ERROR(v_ERR_MSG||':Break-Point Not Found For Template "' || NVL(v_TEMPLATE_NAME, TO_CHAR(p_TEMPLATE_ID)) || '".');
    END IF;
...
BEGIN
	LOGS.START_PROCESS('Calculate WRF for Account',
					   ...
					   p_END_DATE,
                  p_EVENT_LEVEL => CASE WHEN p_TRACE_ON > 0 THEN LOGS.c_LEVEL_DEBUG_MORE_DETAIL ELSE NULL END,
                  p_TRACE_ON => CASE WHEN p_TRACE_ON > 0 THEN 1 ELSE NULL END,
                  p_PERSIST_TRACE => CASE WHEN p_TRACE_ON > 0 THEN TRUE ELSE NULL END
                  );
	...
	LOGS.SET_PROCESS_TARGET_PARAMETER('OUTPUT_AS_OF_DATE', TEXT_UTIL.TO_CHAR_DATE(p_OUTPUT_AS_OF_DATE));
	LOGS.SET_PROCESS_TARGET_PARAMETER('R2_THRESHOLD', TO_CHAR(p_R2_THRESHOLD));
  LOGS.SET_PROCESS_TARGET_PARAMETER('TRACE', TO_CHAR(P_TRACE_ON));
...
BEGIN
    ...
    --Get v_INTERVALS_PER_DAY from profile
--@@Begin Implementation Override --
--@@Ignore The No Data Found Exception Since The Profile May Not Exist And Will Be Created In A Later Step --
   BEGIN
    SELECT NVL(PROFILE_INTERVAL, v_INTERVALS_PER_DAY) INTO v_INTERVALS_PER_DAY
    FROM LOAD_PROFILE WHERE PROFILE_ID = p_PROFILE_ID;
   EXCEPTION
...
--@@Begin Implementation Override --
    ...
    --Assign Calendar to either Meter or Account
--@@Begin Implementation Override --
--@@Defect 1311: prevent the creation of an 'ACCOUNT CALENDAR' when creating a Day Type profile
    IF NVL(TO_CHAR(v_AUTO_APPLY),NVL(GET_DICTIONARY_VALUE('Automatic Calendar Assignment',0,'Profiling','WRF'),'1')) = '1' THEN
    IF p_METER_ID IS NOT NULL THEN
        ASSIGN_METER_CALENDAR(v_PROFILE_LIBRARY_ID,v_CALENDAR_ID,p_METER_ID,TRUE);
    ELSIF p_ACCOUNT_ID IS NOT NULL THEN
        ASSIGN_ACCOUNT_CALENDAR(v_PROFILE_LIBRARY_ID,v_CALENDAR_ID,p_ACCOUNT_ID,TRUE);
	...
	BEGIN
		...
		ELSE
			...
			WHERE A.ACCOUNT_ID = IDs.COLUMN_VALUE
				AND LP.PROFILE_ACCOUNT_REF = TO_CHAR(A.ACCOUNT_ID)
			ORDER BY GET_ACCOUNT_STATION(A.ACCOUNT_ID, p_BEGIN_DATE, p_END_DATE);
