...
--@@Begin Implementation Override
-- Defect 1418: When adding new enrollment row in Load Management, expect the end date of the current row to be automatically updated when saving the new row;
-- end date should be updated to the new row's begin date  minus one calendar day.
   SELECT MAX(END_DATE)
   ...
   UPDATE ACCOUNT_ESP
   SET END_DATE = p_BEGIN_DATE -1
   WHERE ACCOUNT_ID = p_ACCOUNT_ID
   AND END_DATE = v_PREV_END_DATE;
--@@End Implementation Override
