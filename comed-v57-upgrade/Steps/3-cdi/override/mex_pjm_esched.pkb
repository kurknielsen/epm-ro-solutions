...
BEGIN
    RETURN 'Revision: 1.17.5.3.0.0.p2';
END WHAT_VERSION;
---------------------------------------------------------------------------------------------------
...
-- -----------    -----------  ----------------------------------------------------------
-- RMT            05/23/2014   Set status to failure if PJM Response Body is empty.
-- --------------------------------------------------------------------------------------
...
BEGIN
    ...
    -- [RMT 05/23/2014 Case #00211771]:
    -- don't parse unless there are more than six lines to skip Headers and End of Report lines
    IF v_LINES.COUNT > 6 THEN
        -- skip first 5 lines (header info)
        v_IDX := 6;
        WHILE v_LINES.EXISTS(v_IDX) LOOP
            IF v_LINES(v_IDX) IS NOT NULL AND INSTR(v_LINES(v_IDX),g_REPORT_END_LINE) = 0 THEN
                PARSE_UTIL.PARSE_DELIMITED_STRING(v_LINES(v_IDX), ',', v_COLS);
...
-- Person         Date         Comments
-- -----------    -----------  ----------------------------------------------------------
-- RMT            03/12/2019   Updated for PJM MSRS Authentication
-- --------------------------------------------------------------------------------------
...
BEGIN
    ...
    v_PARAMS(MEX_PJM.c_START) := TO_CHAR(p_BEGIN_DATE, 'MM/DD/YYYY');
    v_PARAMS(MEX_PJM.c_STOP) := TO_CHAR(p_END_DATE, 'MM/DD/YYYY');
...
----------------------------------------------------------------------------------------------------
PROCEDURE FETCH_ESCHED_W_WO_LOSSES(
                        p_BEGIN_DATE IN DATE,
                        p_END_DATE   IN DATE,
                        p_CRED       IN   MEX_CREDENTIALS,
                        p_LOGGER     IN  OUT MM_LOGGER_ADAPTER,
    ...
    BEGIN
      v_PARAMS(MEX_PJM.c_Report_Type) := 'Interchange';
      v_PARAMS(MEX_PJM.c_DEBUG) := g_DEBUG_EXCHANGES;
      v_PARAMS(MEX_PJM.c_Report_Name) := 'Load eSchedules With and Without Losses';
      v_PARAMS(MEX_PJM.c_ACTION) := 'download';
...
END FETCH_COMPANIES;
-------------------------------------------------------------------------------
--@@Begin Implementation Override-- Added w/ v5.7 Upgrade
...
PROCEDURE SUBMIT_ESCHEDULE(p_BEGIN_DATE  IN DATE,
  ...
  --'log only' situation?
   ...
   MEX_PJM.RUN_PJM_BROWSERLESS(v_PARAMS,
    ...
    IF p_STATUS = MEX_SWITCHBOARD.c_Status_Success THEN
      p_MESSAGE := 'SUCCESSFULLY UPLOADED';
    ELSE
      p_MESSAGE := 'UPLOAD FAILED';
    END IF;
     DBMS_LOB.FREETEMPORARY(v_RESP);
