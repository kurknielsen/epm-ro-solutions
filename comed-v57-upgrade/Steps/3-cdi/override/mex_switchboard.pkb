...
--
	v_ParmName := p_Parms.first;
    while p_Parms.exists(v_ParmName) LOOP
        -- implementation override jbc 12/30/2015
        -- OpenAM headers are used for authentication, so shouldn't be logged
        IF lower(v_ParmName) LIKE '%openam%' THEN
            cd.build_query_string(g_OTHER_PARM_PREFIX||v_ParmName, p_Parms(v_ParmName), v_Request, v_Request_Headers, TRUE);
        ELSE
		        cd.build_query_string(g_OTHER_PARM_PREFIX||v_ParmName, p_Parms(v_ParmName), v_Request, v_Request_Headers);
        END IF;
