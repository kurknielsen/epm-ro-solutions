...
--@@Begin Implementation Override--
         LOGS.LOG_DEBUG('Calendar Adjustment Value: ' || TO_CHAR(v_CALENDAR_ADJUSTMENT.ADJ_VAL) || ', Operation: ' || CASE WHEN v_CALENDAR_ADJUSTMENT.ADJ_OP = 1 THEN 'Add' ELSE 'Multiply' END);
...
v_INDEX BINARY_INTEGER;
--@@Begin Implementation Override--
   FUNCTION USE_PRIMARY_PROFILE(p_PROFILE IN GA.NUMBER_TABLE) RETURN BOOLEAN AS
   v_PROFILE_INDEX BINARY_INTEGER;
   v_ZERO_COUNT PLS_INTEGER := 0;
   BEGIN
      v_PROFILE_INDEX := p_PROFILE.FIRST;
      WHILE v_PROFILE_INDEX <= p_PROFILE.LAST LOOP
         v_ZERO_COUNT := v_ZERO_COUNT + CASE WHEN p_PROFILE(v_PROFILE_INDEX) = 0 THEN 1 ELSE 0 END;
         v_PROFILE_INDEX := p_PROFILE.NEXT(v_PROFILE_INDEX);
      END LOOP;
      RETURN v_ZERO_COUNT = 0; 
   END USE_PRIMARY_PROFILE;
--@@End Implementation Override--
...
--@@Begin Implementation Override--
--@@For Calendars With Multiple Load Profile Assigments, Use The First Load Profile If It Is Complete. If Not Complete Then Clear The Composite And Continue With The Other Profiles --
         IF v_PROFILE_IDS.COUNT > 1 AND v_ASSIGNMENT_TYPE = 'B' THEN
            IF v_INDEX = v_PROFILE_IDS.FIRST THEN 
               LOGS.LOG_DEBUG('Calendar: ' || GET_CALENDAR_NAME(p_CALENDAR_ID) || ', Profile Type: ' || p_PROFILE_TYPE || ', Profile Source Date: ' || NVL(TO_CHAR(p_PROFILE_SOURCE_DATE, 'MM/DD/YYYY'),'?') || ', Service Date: ' || TO_CHAR(p_CAST_CONTEXT.SERVICE_DATE, 'MM/DD/YYYY'));
               IF CASE WHEN p_PROFILE_TYPE = g_HISTORICAL_PROFILE AND p_PROFILE_SOURCE_DATE <> p_CAST_CONTEXT.SERVICE_DATE THEN FALSE ELSE TRUE END AND USE_PRIMARY_PROFILE(p_COMPOSITE_PROFILE) THEN
                  LOGS.LOG_DEBUG('Primary Load Profile "' || FW.GET_PROFILE_NAME(v_PROFILE_IDS(v_INDEX)) || '" Selected For ' || TO_CHAR(p_CAST_CONTEXT.SERVICE_DATE, 'MM/DD/YYYY'));
                  EXIT;
               END IF;
               p_COMPOSITE_PROFILE.DELETE;
               LOGS.LOG_DEBUG('Secondary Load Profile "' || FW.GET_PROFILE_NAME(v_PROFILE_IDS(v_INDEX)) || '" Selected For ' || TO_CHAR(p_CAST_CONTEXT.SERVICE_DATE, 'MM/DD/YYYY'));
            END IF;
         END IF;
--@@End Implementation Override--
...
--@@Begin Implementation Override--
      LOGS.LOG_WARN('Zero Profile Default Assigned To Calendar "' || GET_CALENDAR_NAME(p_CALENDAR_ID) || '" On ' || TO_CHAR(p_CAST_CONTEXT.PROFILE_DATE,'MM/DD/YYYY'));
--@@End Implementation Override--
	END IF;
		...
		WHEN NO_DATA_FOUND THEN
			...
			IF SQL%NOTFOUND THEN
				...
				VALUES(
					...
					SYSDATE);
			END IF;
--@@Begin Implementation Override:  look up the unique PROFILE_ID for a given PROFILE_NAME
      WHEN TOO_MANY_ROWS THEN
         BEGIN
            SELECT A.PROFILE_ID
            INTO p_PROFILE_ID
            FROM LOAD_PROFILE A, CALENDAR_PROFILE B, CALENDAR C
            WHERE A.PROFILE_NAME = p_PROFILE_NAME
            AND C.CALENDAR_NAME = p_PROFILE_NAME
         ...
         EXCEPTION
            WHEN OTHERS THEN
               ...
               AND ROWNUM = 1;
         END;
--@@End Implementation Override:  look up the unique PROFILE_ID for a given PROFILE_NAME
...
--@@Begin Implementation Override
   PUT_SETTLEMENT_PROFILE(p_PROFILE_NAME, v_PROFILE_LIBRARY_ID, p_ACCOUNT_ID, p_METER_ID, p_INTERVAL, p_CAST_CONTEXT.SERVICE_DATE ,v_PROFILE_ID);
--@@End Implementation Override
...
BEGIN
	...
	IF GA.VERSION_ACTUAL THEN
		...
		WHERE A.MODEL_ID = p_CAST_CONTEXT.MODEL_ID
			...
			AND A.PROVIDER_SERVICE_ID = p_PROVIDER_SERVICE_ID
