CREATE OR REPLACE PACKAGE BODY CDI_PJM AS
          ...
          FROM
             ...
             FROM
                ...
                (SELECT DISTINCT TRANSACTION_ID, SERVICE_TYPE, SUPPLIER_TYPE, SUPPLIER_NAME, PJM_SHORT_NAME
                   FROM CDI_COMED_PLC_NSPL_SCHEDULE$ WHERE SCHEDULE_DATE BETWEEN v_BEGIN_DATE - 1 AND v_END_DATE AND (SERVICE_TYPE = p_SERVICE_TYPE OR CONSTANTS.ALL_STRING = p_SERVICE_TYPE))) A,
                      CDI_COMED_PLC_NSPL_SCHEDULE$ B
...
CURSOR c_VALIDATE IS
   ...
   FROM
      ...
      FROM
         ...
         FROM
            (SELECT A.SERVICE_TYPE, A.SUPPLIER_TYPE, A.SUPPLIER_NAME, A.PJM_SHORT_NAME, A.SCHEDULE_DATE, B.INTERNAL_AMOUNT
            FROM (SELECT LOCAL_DAY "SCHEDULE_DATE", TRANSACTION_ID, SERVICE_TYPE, SUPPLIER_TYPE, SUPPLIER_NAME, PJM_SHORT_NAME FROM (SELECT LOCAL_DAY FROM CDI_LOCAL_DAY$ WHERE LOCAL_DAY BETWEEN v_BEGIN_DATE - 1 AND v_END_DATE), (SELECT DISTINCT TRANSACTION_ID, SERVICE_TYPE, SUPPLIER_TYPE, SUPPLIER_NAME, PJM_SHORT_NAME FROM CDI_COMED_PLC_NSPL_SCHEDULE$ WHERE SCHEDULE_DATE BETWEEN v_BEGIN_DATE - 1 AND v_END_DATE AND SERVICE_TYPE IN (CONSTANTS.ALL_STRING, p_SERVICE_TYPE))) A, CDI_COMED_PLC_NSPL_SCHEDULE$ B
            WHERE B.SERVICE_TYPE(+) = A.SERVICE_TYPE
               AND B.TRANSACTION_ID(+) = A.TRANSACTION_ID
               AND B.SUPPLIER_TYPE(+) = A.SUPPLIER_TYPE
               AND B.SUPPLIER_NAME(+) = A.SUPPLIER_NAME
...
-- Select Internal Schedules Defined Over The Specified Date Range --
   ...
   FROM CDI_COMED_PLC_NSPL_SCHEDULE$
...
-- Load Zone Demand --
--KN      MM_PJM_POWERMETER.QUERY_POWERMETER(p_BEGIN_DATE, p_END_DATE, c_QUERY_FINAL_ZONE_LOAD, NULL, NULL, p_TRACE_ON, p_STATUS, p_MESSAGE);
--KN      IF p_STATUS = GA.SUCCESS THEN
--KN         POST_SYSTEM_LOAD_WITH_LOSSES(p_BEGIN_DATE, p_END_DATE);
--KN      ELSE
--KN         LOGS.LOG_WARN('System Load With Losses Were Not Posted. The Query Of Load Submission Content From The Exchange Returned A Non-Zero Status: ' || TO_CHAR(p_STATUS));
--KN      END IF;
-- 500kv Losses --
--KN      MM_PJM_POWERMETER.QUERY_POWERMETER(p_BEGIN_DATE, p_END_DATE, c_QUERY_500KV_LOSSES, NULL, NULL, p_TRACE_ON, p_STATUS, p_MESSAGE);
   ELSE
