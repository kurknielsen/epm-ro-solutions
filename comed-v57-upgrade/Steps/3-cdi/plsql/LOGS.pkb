...
BEGIN
	...
	-- If this is not a parallel child, then set the process Id to NULL to indicate a user session.
	-- A User Session can change over time or if/when
	-- SECURITY_CONTROLS.SET_CURRENT_USER is invoked. The non-null ID is dynamically
	-- determined every time CURRENT_PROCESS_ID, below, is invoked.
	-- However, a parallel child does not have a "User Session" process, so we just set its process id directly.
	IF v_PARENT_SID IS NULL THEN v_PROCESS_ID := NULL; END IF;
...
--			GET_CALLER(v_PROCEDURE_NAME, v_STEP_NAME, 3);
			GET_CALLER(v_PROCEDURE_NAME, v_STEP_NAME, 2); -- @@Implemention Override: Setting from v5.3 (KN)
